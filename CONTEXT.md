...
Per **homeworld sector**, the set of possible **homeworld owner** slots from **homeworld ownership evidence**, each carrying a provenance collection (machine facts: kind, turn, strength class, and signal-specific ids/distances). Empty when unknown; membership and uniqueness for display are **derived** by **homeworld evidence strength** resolution. Surfaced on sector region-overlay wire facts for hover; not itself a confidence tier and not candidate wire `perspective` (though a uniquely resolved owner may bind orphan candidates for layout-prior / pin display).
