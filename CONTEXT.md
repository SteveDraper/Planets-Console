...
Bootstrap metadata tuple (`TURN_ANALYTIC_CATALOG` in `api/analytics/catalog.py`) of turn analytic ids and SPA-facing metadata (`name`, `supports_table`, `supports_map`, `type`). BFF and `catalog_entry()` read this bootstrap catalog without importing Core compute. Core **turn analytic registration** objects (`TURN_ANALYTIC_REGISTRATIONS` in `registry.py`) reference bootstrap entries via `catalog_entry(analytic_id)`; `registry.py` derives the live catalog tuple from registrations and validates it matches the bootstrap via `publish_turn_analytic_catalog`. `TurnAnalyticCatalogEntry`, `catalog_entry()`, and catalog-order alignment helpers live in `api/analytics/catalog.py`. BFF descriptor registries import `TURN_ANALYTIC_CATALOG` from `catalog.py` and are validated against it at import. Adding an analytic requires: one bootstrap `TurnAnalyticCatalogEntry` in `catalog.py`, one Core **registration** (catalog entry reference + context `compute` handler + export catalog placeholder), append to `TURN_ANALYTIC_REGISTRATIONS` in `registry.py`, one BFF `_BFF_DESCRIPTORS_BY_ID` entry (table/map shaping), and -- once exports ship (#95) -- wiring the registration's export catalog into the export registry. `TURN_ANALYTICS` is derived at import from registrations. The catalog still prevents metadata and id drift between Core and BFF.
_Avoid_: editing derived `TURN_ANALYTICS` directly (append one registration instead); maintaining separate catalog and handler maps in Core; merging Core handlers with BFF shapers into one cross-layer list; inline duplicate catalog metadata in registrations instead of bootstrap + `catalog_entry()`
