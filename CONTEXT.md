...
NDJSON wire protocol (**#71**, Phase 1H). The SPA opens **one multiplexed table stream** (`GET .../inference/table-stream`) for all scoreboard rows on the current shell scope; events carry an optional `playerId` tag (except `globalPause`). Per-row `/inference/stream` remains for **inference row resume** after global pause or per-row halt. Emits whenever a **new** **inference explanation signature** is admitted to **inference merged top-K** -- within-tier enumeration and cross-tier ladder progress -- so the hourglass clears before top-K is full. Each `solution` event carries the **full held top-K** for that row (ranked by **inference solution rank weight**); the consumer replaces local held state from the event (no client-side merge). Follows the load-all progress stream pattern (Zod-owned events). Batch JSON remains for the inference corpus harness.
_Avoid_: websocket inference

**Inference table stream**:
Single NDJSON connection for all scoreboard rows on one game / turn / **perspective** scope. The backend multiplexes per-row scheduler event queues onto one wire; the frontend demuxes on `playerId`. Chosen over parallel per-row HTTP connections so **inference global pause** can freeze all rows on one scope without juggling N client abort controllers. Per-row halt still targets one player via `POST .../inference/stop` without closing the table stream.
_Avoid_: turn-level inference job queue in the SPA

**Inference global pause**:
User-initiated freeze of all in-flight build inference for the current shell scope (game, turn, **perspective**). **Pause** drains the scheduler worker queue into a held buffer and broadcasts `globalPause` on open streams; rows already held in top-K stay visible with `paused` chrome. **Resume** requeues held tier jobs and per-row continuations. Distinct from **inference row halt** (one row only). Implicit scope change cancels everything (no pause preservation across scopes).
_Avoid_: confusing global pause with per-row halt

**Inference row scheduler**:
Process-wide backend facility (**#71** companion) that fair-schedules **inference search tier** work across concurrent inference streams (table-multiplexed or per-row). One schedulable **inference tier job** = one full **inference search tier** step for one scoreboard row (catalog build, exact/band passes, within-tier top-K, merge admit, seed output). Each row's ladder is sequential (tier *n+1* needs tier *n* job outputs). Cross-row fairness: enqueue tier-1 when a row is scheduled, append tier *n+1* after tier *n* completes, shared FIFO queue drained by a worker pool (default **4** workers, configurable). Distinct from **inference corpus runner** `--workers` (batch case parallelism). `solution` events emit from inside the tier job via merge-admit hooks. Integrates **inference global pause** (held jobs, session preservation on table-stream disconnect while paused).
_Avoid_: frontend job queue, per-solve queue jobs

**Inference solve interrupt boundary**:
Where **inference row halt** and stream cancellation can take effect without losing tier progress. v1 (**#71**): cooperative checks at sub-step boundaries inside a tier job -- top-K no-good iterations, seed attempts, exact vs band passes -- plus `StopSearch()` in a CP-SAT callback when halt fires mid-`Solve()`. OR-Tools CP-SAT cannot resume internal search state across `Solve()` calls; only complete solutions can warm-start the next pass via hints. **Known gap:** a single long first-feasible `Solve()` on a huge catalog may block halt until that call returns. **Follow-on (if needed):** retry `UNKNOWN` sub-steps until feasible or halt (logical continuation; CP-SAT restarts internally each retry).
_Avoid_: routine short-solve slice loops (wastes search), assuming CP-SAT pause/resume

**Inference solution count indicator**:
Per scoreboard row chrome replacing the binary green tick: a green outlined badge showing **N** = the number of rows currently held in **inference merged top-K** (not cumulative discoveries above K). **N > 0** as soon as the first exact explanation is held; rises toward K then plateaus while eviction swaps membership. Hourglass while **N = 0** and search is in flight; red cross when the row completes with no exact explanation. While search is in flight, a per-row **halt control** cancels that row's stream and tier jobs. Click on the badge (or row chrome when **N > 0**) opens the ranked modal.
_Avoid_: green tick, checkmark column

**Accelerated-start inference row** (SPA):
A scoreboard row whose host turn falls in an accelerated-start window may run multiple internal segments (accel window + reported host turn). v1 **#71** uses the same per-row NDJSON stream and **inference row scheduler** as other rows; segments stay inside the row's inference path with no per-segment SPA time split (user **inference row halt** or natural completion ends the row).
_Avoid_: per-segment stream, segment-level halt (v1)

**Inference row halt**:
User-initiated or implicit cancellation of one scoreboard row's in-flight build inference. **Explicit:** per-row halt control in the inference column. **Implicit:** shell scope change (game, turn, **perspective**), disabling build inference, or client disconnect -- same cancellation for that row's stream. The **inference row scheduler** drops queued and in-flight **inference tier jobs** for the cancelled run. Terminal `complete` uses `status: stopped`, `isComplete: true`, and preserves any rows already held in **inference merged top-K** (`solutionCount` = held **N**). Distinct from failure (`no_exact_solution`, solver error), from natural completion (`exact` when final-catalog equalities pass), and from **inference global pause** (all rows frozen, partial state kept, resumable).
_Avoid_: time_limited (as the primary stop mechanism once **#71** ships)
