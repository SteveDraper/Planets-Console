...
Later-turn signals that strengthen **where** a **homeworld planet** is (candidate planet confidence), not **who** owns which **homeworld sector**. v1: **ship origin distance signal** + **single-starbase new-build evidence** on existing candidates only. Only single-starbase new-build flips **possible → definite**.
...
Later-turn signals that strengthen **homeworld owner** attribution for **homeworld sectors**. v1: ship travel envelopes from fleet `built_turn` (via orchestrator **ENSURE** on final `fleet@N` for **all roster players** -- not a soft read) or ship-id/scoreboard max age (`turns × warp²`; unknown engines → W9; gravitonic hulls only; ignore HYP-capable hulls); sector reachable-set reduction that adds an owner when only one sector remains; preferred-candidate and nearby (≤162 LY) planetary `ownerid` sightings that **add** slots to a sector's **possible homeworld owner set**, each with a provenance collection. Distinct from **homeworld location evidence**. Conflicts stay multi-member for SPA display (**ambiguous**); [#37](https://github.com/SteveDraper/Planets-Console/issues/37) may add decisive `user_asserted` provenance. Does not invent location candidates or flip confidence tier alone.
_Avoid_: soft-reading fleet without a DAG wake, depending on fleet with `player_id=None` (always-satisfied no-op), treating origin-distance tier promotion as ownership assignment, calling sector ownership "perspective", collapsing multi-owner evidence to a single forced pin, using FC=`HYP` alone as the HYP capability test

**Homeworld sector owner set**:
Per **homeworld sector**, the set of possible **homeworld owner** slots inferred from **homeworld ownership evidence**, each carrying a provenance collection (machine facts: kind, turn, and signal-specific ids/distances). Empty when unknown; size 1 when uniquely attributed; size >1 when ambiguous. Surfaced on sector region-overlay wire facts for hover; not a confidence tier and not candidate wire `perspective` (though unique owners may bind orphan candidates for layout-prior / pin display).
_Avoid_: single forced owner when evidence conflicts, English hover strings in Core, storing sector ownership only as candidate.perspective
...
How strongly the locator treats a candidate as the **homeworld planet** for a slot or orphan location. **Definite** when a baseline profile matches, geometry leaves no plausible alternative, **single-starbase new-build evidence** promotes the candidate, or the user manually confirms. **Possible** when consistent with settings and spacing but not unique. Origin-distance observations do not change tier. Orphans default to **possible** until anchored or confirmed. Orthogonal to **homeworld selection status** -- layout-prior ranking does not create a third confidence tier.
_Avoid_: most probable as a confidence tier, probability score as the tier enum, cumulative origin-distance as definite
