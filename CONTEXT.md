...
Provenance label on scores inference `complete` diagnostics (`diagnostics.fleetTorpInputStatus`) for whether prior-turn fleet overlay input was authoritative when a row finished: `not_applicable` (host turn 1), `pending` (prior-turn RST present but `fleet@(host_turn - 1)` snapshot not yet available at schedule time), `applied` (overlay from persisted prior-turn fleet), `unavailable` (fleet services missing). Distinct from row `displayStatus` / solver completion. When `applied`, `diagnostics.fleetTorpOverlay.beliefSetTorpIds` may list the belief-set torp ids used. First-pass rows may complete with `pending` while background warm or export ensure materializes fleet; persisting a player's ensure-final `fleet@(N - 1)` ledger invalidates that player's scores@N row and reschedules their open stream row so a later `complete` can report `applied` without reconnect. SPA shows a scope banner and badge affordance while any row is `pending`; skipping the banner is acceptable if another gap-fill caller finishes first (#158).
