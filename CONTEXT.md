...
Client preference (localStorage) for which **homeworld region overlay** sector outlines appear: a sticky preset **Pinned / Unpinned / Selected** (default **Selected**) plus, when preset is **Selected**, an optional explicit multi-select of sector indexes (``null`` = all current sectors). **Pinned** / **Unpinned** derive the visible set from overlay facts at paint/panel read time (indexes are not stored for those presets). Switching preset from **Pinned** or **Unpinned** to **Selected** materializes that effective set into stored indexes. Any direct sector-tile toggle forces **Selected** and writes an explicit set. Same sticky preference pattern as **Cartography layer** toggles; independent of **Visibility region kind** enablement. Replaces the older **homeworld region display mode** (`off` / `un-pinned` / `pinned` / `all`).
