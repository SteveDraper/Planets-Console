...
The SPA plugin for one **turn analytic**'s **Shell** chrome, keyed by **turn analytic catalog** id. Optional slots: sidebar factory (`null` = generic checkbox for that **view mode**), MainArea table view, generic-route query-param adapters, GameInfo availability, and a table-stream hook (**shell-lived stream** or **tile-lived stream**, optional React context for shell-lived). Sparse: no registration means generic checkbox and generic table, not an error. Distinct from Core turn analytic registration, **analytic descriptor**, and the map fetch/merge registry. See [ADR 0026](docs/adr/0026-shell-analytic-registration.md).
