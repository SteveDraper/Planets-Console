...
**Fleet heading trail**:
Optional map rays for one **fleet ship record** with known motion: player-colored segments from the ship's **fleet location ring** origin along heading, length one-turn travel in ly per segment (`travelLyPerTurn` = warp², ×2 for gravitonic hulls). Wire-only `motion` on table/stream records (`heading`, `warp`, `travelLyPerTurn`, `trailStop`); omitted when not underway or heading/speed unknown (parked target==position, inferred / never-sighted). `trailStop` is always present when `motion` is, and clamps forward multi-turn extension at the waypoint, or at the planet when warp ≥ 2 and the waypoint is on/in a normal warp well (W1 is not pulled into wells, so trailStop stays at the waypoint). Sidebar **Trail** extend `N` ∈ 0..5 (localStorage): `0` = current-turn only; `N > 0` adds forward and backward segments with opacity by `|turnOffset|`. [#290](https://github.com/SteveDraper/Planets-Console/issues/290).
_Avoid_: durable ledger motion fields, guessing heading for inferred-only ships, full movement simulation (HYP / fuel / intercept) in v1
