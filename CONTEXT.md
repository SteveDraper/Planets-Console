...
A later-turn signal that increases or decreases confidence in a **homeworld candidate** without replacing the **homeworld inference baseline**. For the location slice (#36): **ship origin distance signal** (soft / non-promoting hits) and **single-starbase new-build evidence** (hard definite). Ordinary hits: at most one independent hit per (**planet**, turn). Ownership / slot-binding evidence (ship proximity envelopes, sector ownership from max travel, direct planetary ownership sightings) is a separate analytic concern -- not #36.
...
One countable origin-distance / sighting unit for a candidate planet: at most one per (**planet**, turn), regardless of how many ships or signal kinds matched that turn. Does **not** flip **homeworld confidence tier**. Distinct from **single-starbase new-build evidence**, which is the only automatic hard definite path.
_Avoid_: per-ship hit counting, per-kind-per-turn splitting as the v1 default, treating hit count as definite promotion
