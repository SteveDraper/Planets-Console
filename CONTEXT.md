...
Tagged unexplained military on an inference row when a **placeholder departure** of unknown military is mixed into the observation. Either today's **point leftover** (`unexplainedMilitaryDelta2x`) or an **unknown-loss bound** -- `lower_bound_2x = max(0, observed_drop_2x − max_possible_lost_construction_2x)` -- a lower bound on actual unexplained military, not a construction envelope on the placeholder. Persist, stream, and chrome distinguish the tag from a point leftover.
