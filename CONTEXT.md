...
**Fleet map layer**:
The **map layer** contribution of the **fleet analytic** for one shell turn. Projects **fleet player visibility**-enabled **`active`** **fleet ship record**s whose **`lastSeen.turn` equals the shell turn** (and that have a known last-seen point) into **fleet location ring**s (not one marker per ship). Region-only geometry (**fleet field constraint** **region** / starbase overlays) remains deferred. Rows with no known `x/y`, or with a stale `lastSeen` from an earlier turn, are excluded.
