...
Shared declarative list (`TURN_ANALYTIC_CATALOG` in `api/analytics/catalog.py`, published from `api/analytics/registry.py` at import) of turn analytic ids and SPA-facing metadata (`name`, `supports_table`, `supports_map`, `type`). Derived from Core **turn analytic registration** objects (`TURN_ANALYTIC_REGISTRATIONS` in `registry.py`). `TurnAnalyticCatalogEntry`, `catalog_entry()`, and catalog-order alignment helpers live in `api/analytics/catalog.py`. BFF descriptor registries are validated against the catalog at import. Adding an analytic requires: one Core **registration** (catalog entry + context `compute` handler + export catalog placeholder), append to `TURN_ANALYTIC_REGISTRATIONS` in `registry.py`, one BFF `_BFF_DESCRIPTORS_BY_ID` entry (table/map shaping), and -- once exports ship (#95) -- wiring the registration's export catalog into the export registry. `TURN_ANALYTIC_CATALOG` and `TURN_ANALYTICS` are derived at import; do not edit them directly. The catalog still prevents metadata and id drift between Core and BFF. BFF imports `catalog_entry` from `catalog.py`, not `registry.py`, so metadata lookups stay lightweight until registry publish runs.
_Avoid_: editing `TURN_ANALYTIC_CATALOG` or `TURN_ANALYTICS` directly (append one registration instead); maintaining separate catalog and handler maps in Core; merging Core handlers with BFF shapers into one cross-layer list
