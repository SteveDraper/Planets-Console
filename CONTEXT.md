...
**Departure pin**:
Determination that every unit implied by a **count-lattice signal** for a class is identified well enough to price that class, after **fleet alibi** cull of this-turn still-owned known `shipId`s (viewpoint `TurnInfo.ships`, not fleet@T). Every remaining same-class prior row has **known spec**; either remainder count equals the class drop (that set left, mixed hulls allowed) or remainders share one spec so the drop's military is determined without naming ids. Any same-class remainder that lacks known spec fails the pin. Military SAT is refused only when a signaled class that can move military (warships, or unknown class) is unpinned; an unpinned **true-freighter** drop does not refuse SAT. An exact-set pin names the records that fleet@T retires; a spec-only pin does not. Combat and explosion reports are extras, not this gate ([#485](https://github.com/SteveDraper/Planets-Console/issues/485)).
_Avoid_: unique-fill, `len(decrease_candidates)==1`, last-turn inference singleton, hull-only alibi, waiting on current-turn fleet persist, refusing SAT for a 0-military freighter-only unpin, FIFO-retiring identical remainders
