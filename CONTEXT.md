...
Later-turn signals that strengthen **homeworld owner** attribution for **homeworld sectors**. v1: ship travel envelopes from fleet `built_turn` (via orchestrator **ENSURE** on final `fleet@N` for **all roster players** -- not a soft read) or ship-id/scoreboard max age (`turns × warp²`; unknown engines → W9; gravitonic hulls only; ignore HYP-capable hulls); sector reachable-set reduction that adds an owner when only one sector remains; preferred-candidate and nearby (≤162 LY) planetary `ownerid` sightings that **add** slots to a sector's **possible homeworld owner set**, each with a provenance collection. Distinct from **homeworld location evidence**. Each provenance kind maps to a **homeworld evidence strength class**; display/decision state is derived from those lists (not a separate ownership attribution flag). Does not invent location candidates.
