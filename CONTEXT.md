...
The torpedo type ids counted as "in fleet" for **inference aggregate admission** -- derived from prior-turn fleet ledger records feeding #87 (known launchers plus **fleet build option set** unions). For ambiguous rows, membership is the **union** of launcher/torp ids across all option sets on active rows (consistent tuples only, no per-field Cartesian product). Empty when no active row carries a positive launcher/torp type in any option set. Distinct from **inference fleet launcher belief mass**, which drives **inference torp misalignment penalty** ranking. The fleet export `$.composition` histogram branch is not the source of truth for this set (pending removal: [#252](https://github.com/SteveDraper/Planets-Console/issues/252)).
_Avoid_: launcherTypes (wire field name in prose unless citing path), known-only fleet composition, top-1 option set only, using admission membership alone as the misalignment scale

**Inference fleet launcher belief mass** (#87):
Per torp-id strength of launcher support in prior-turn fleet evidence used to scale **inference torp misalignment penalty**. **Hard** support is a known positive launcher type on an active row (`fields.launchers` known with id > 0); that id has full mass from that evidence. **Soft** support on an ambiguous row is the softmax probability mass over that row's **fleet build option set**s, using each set's **inference solution rank weight** divided by **`INFERENCE_PROBABILITY_WEIGHT_SCALE`** (100) as the log-score, summed over sets whose `torp_id` equals the torp id -- beam-only / no-tube sets still participate in the softmax so they can starve tube types. Option sets never count as hard even if unanimous. Player-level mass for a torp id is the **max** over active rows (hard or soft). Admission still uses the flat **inference fleet launcher belief set** union; mass does not remove types from early-tier admission. The same per-option-set softmax weights also gate which soft fits count toward fleet max-tech ceilings (see **inference component tech-gap prior**).
...
Minimum per-option-set softmax probability (on its row, after `rank_weight / SCALE` softmax) required for that **fleet build option set**'s components to contribute to fleet max-tech ceilings used by **inference component tech-gap prior** and prior-fleet tech raise. Tunable as **`fleetInferenceTuning.optionSetMassThreshold`** (default **0.25**). Distinct from **inference fleet launcher belief mass** (torp-id aggregates for misalignment).
_Avoid_: conflating with torp belief mass, using rank-weight deltas instead of softmax probability
