...
One feasible fitted-ship specification -- hull + engine + beam + launcher types and slot fills as a consistent tuple. On **fleet inferred acquisition** rows, entries come from **military score build inference** held `shipBuilds`; when top-K solutions disagree, the row exposes a list of option sets (not per-field Cartesian products). UI may show the highest-**inference solution rank weight** set as the display default while listing alternates. On **fleet observed ship** rows, ingest attaches a **single confirmed** option set so beam/launcher slot fills (`ship.beams` / `ship.torps`) are available for display while `fields.beams` / `fields.launchers` stay type-id constraints. Observation reconciliation matches a sighting to at most one option set before collapsing fields to **known**.
