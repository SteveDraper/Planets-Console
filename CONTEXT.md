...
The set of one position per participating **homeworld sector** that minimizes an equal-family-weighted layout-prior cost using the **homeworld layout distribution asset** for the game category plus soft **origin-distance observation** evidence. Cost = mean over ring edges of `-log` Normal density of clockwise-neighbor separation plus mean over unpinned set members of `-log` Normal density of map-center distance (asset `mean`/`std` per family) plus λ-blended soft evidence `E(S)` from persisted origin-distance observations (`P(o|S)=|S∩M|/|M|`, turn means, `E=(E+λe)/(1+λ)`, default λ=`origin_distance_evidence_lambda` 0.8; stand-ins do not absorb credit). Clockwise-neighbor means angle-sorted around map center with Euclidean distance to the next on the ring. Per sector the set member is: the **definite** planet when pinned; else a real **possible** planet (**most probable homeworld candidate** when chosen); else a **homeworld layout stand-in** when the sector has planet-scan-unobserved band area; else the sector does not participate. Pinned **definite** members are fixed in the set and omitted from the center-distance family. Selection is a **budgeted anytime** solve: a configurable computation budget must yield a feasible selection promptly, and additional budget may only improve or leave unchanged the incumbent cost (exact global optimum when budget allows is a goal, not a hard requirement on every request). On exact cost ties, prefer the lexicographically smaller tuple of selected planet ids (stand-in-only sectors sort after real planets). Same inputs and **`LAYOUT_PRIOR_ALGORITHM_VERSION`** must yield the same selected planet-id set (search may be stochastic only under a seed derived from those stable inputs). Opinionated layout prior -- not evidence promotion. Eligible only under the same gate as **homeworld region overlay** emission: **homeworld region geometry (v1)** (`hwdistribution=2`, `mapshape=0`), game category epic|standard, and a viewpoint pin fixing ring rotation; otherwise no **most probable homeworld candidate** annotations. Shell-turn result is versioned-persisted (see **homeworld selection status**); algorithm or cost/stand-in/tie-break policy changes bump `LAYOUT_PRIOR_ALGORITHM_VERSION`. See [ADR 0009](docs/adr/0009-homeworld-layout-prior-budgeted-solver.md).
