...
Client preference (localStorage) for which **homeworld region overlay** sector outlines appear: sticky UI presets **Pinned / Unpinned / Selected**, plus an init-only internal preset **``all``** (UI still shows **Selected**) used until overlays are ready. On overlays-ready, ``all`` materializes to **Selected** + an explicit full sector-index list. **Pinned** / **Unpinned** / **Selected** always rewrite that concrete index list (Pinned/Unpinned from overlay facts; Selected keeps the current effective set when entered from another preset). Direct sector-tile toggles force **Selected** and write an explicit set. ``useHomeworldRegionSelection`` owns the overlay query and materialize writes; the Zustand store only holds preset + indexes + envelope checkbox. Same sticky preference pattern as **Cartography layer** toggles; independent of **Visibility region kind** enablement. Replaces the older **homeworld region display mode** (`off` / `un-pinned` / `pinned` / `all`).
_Avoid_: visibility kind toggle (wrong analytic), per-game preference scope for this mode, treating assert-focus single selection as the outline multi-select, surfacing internal ``all`` on the Region selection control, ``null`` indexes as a second "all" encoding
