...
Per **homeworld sector**, the set of possible **homeworld owner** slots from **homeworld ownership evidence**, each carrying a provenance collection (machine facts: kind, turn, strength class, and signal-specific ids/distances). Empty when unknown; membership and uniqueness for **display** are **derived** by **homeworld evidence strength** resolution plus a display-time cross-sector trim (slots uniquely settled strong/asserted, or definite slot-anchored location pins, are omitted from other sectors' overlay sets -- durable aggregates keep full membership so asserts can change without sticky holes). Surfaced on sector region-overlay wire facts for hover/title; not itself a confidence tier and not candidate wire `perspective` (though a uniquely resolved owner may bind orphan candidates for layout-prior / pin display).
_Avoid_: single forced owner when same-strength evidence conflicts, English hover strings in Core, storing sector ownership only as candidate.perspective, treating the set as independent of provenance strength
...
_Avoid_: visibility kind toggle (wrong analytic), per-game preference scope for this mode, treating assert-focus single selection as the outline multi-select, surfacing internal ``all`` on the Region selection control, ``null`` indexes as a second "all" encoding, continuous store sync of Pinned/Unpinned indexes, attributing overlay fetch or materialize ownership to ``useHomeworldRegionSelection``
