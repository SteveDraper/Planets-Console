.PHONY: test lint ci ci_full typecheck_frontend check_frontend_api_slices check_frontend_api_no_monolithic_schema test_bff test_api test_api_full test_server test_mcp_adapter test_scripts test_frontend generate generate_frontend_api inference_corpus inference_corpus_discover inference_corpus_probe bundle_console_package wrap_console_package console_package_probe release
...
# Wrap bundler output into a GitHub Release .dmg (macOS) or Inno setup .exe (Windows).
wrap_console_package:
	uv sync --extra package --extra dev
