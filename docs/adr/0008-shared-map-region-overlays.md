...
- **Geometry is discriminated:**
  ...
  - **`boundary`** -- closed ordered vertices with per-edge `line` | `arc` (map-space clockwise on arcs). Optional colocated envelope **disks** on the same entry (e.g. 81/162 LY homeworld envelopes). Suitable for annular sectors.
- **Optional shared annotations** on any overlay: `isPinned`, `status`, `hoverSummary`. Analytics that do not use them omit the fields.
