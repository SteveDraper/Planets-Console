...
- **Optional shared annotations** on any overlay are **domain/machine facts only**: `isPinned`, `status`, `candidateCount`, `playerLabel` (roster identity, not a sentence). Analytics that do not use them omit the fields. **UI copy** (hover tooltip lines, labels) is assembled by the **client** (preferred) or BFF from those facts -- Core does not emit English `hoverSummary` or other presentation strings.
