...
12. **Phased rollout** -- v1: export ensure + stream steps; phase 2: batch `compute()`; phase 3: BFF (HTTP) uniform compute API. The **MCP adapter** is an in-process Core caller of **analytic export ensure**, not a phase-3 API consumer.

## Amendment ([How this MCP product relates to orchestrator phase 3](https://github.com/SteveDraper/Planets-Console/issues/320))

Supersedes the phase 3 "BFF/MCP uniform API" wording in locked choice 12. Phase 3 ([Compute orchestrator (phase 3): uniform BFF compute API](https://github.com/SteveDraper/Planets-Console/issues/203)) is **BFF (and other HTTP callers)** only: Core `ComputeRequest` / `ComputeResult` transport and BFF proxy. The **MCP adapter** is a standing in-process Core *caller* of **analytic export ensure** (orchestrator via [#204](https://github.com/SteveDraper/Planets-Console/issues/204)). MCP never exposes `ComputeRequest` / `ComputeResult` as a tool or resource. Hatch `ensure_analytic_export` does not wait on phase 3.
