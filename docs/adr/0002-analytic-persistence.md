...
- **Compute:** Issue #34 registers baseline ensure with the **compute orchestrator**. [#36](https://github.com/SteveDraper/Planets-Console/issues/36) phase 2 adds the linear self-chain (`homeworld-locator@T` depends on `homeworld-locator@(T−1)`) and refines evidence aggregates through the shell turn from the baseline floor. Baseline ensure writes game-global candidates + floor evidence aggregate at the baseline turn (turn 1, or degraded earliest).
