...
- **Write gate (shipped, #366):** persist terminal `complete` outcomes with functional status `exact`, `no_exact_solution`, `moderate_residual`, or `mine_score_residual`; also fallback-complete **inference admission skip** statuses (`viewpoint_owner`, `dead`, `full_alliance`, `horwasp`, `no_prior_turn`, `player_not_found`) plus `invalid_problem` / `solver_error`, and `stopped` / `time_limited` for turn-evidence close. See [Inference product-status persist and stream contract](https://github.com/SteveDraper/Planets-Console/issues/360) / design §3.8. Do not persist `fetch_error`, in-progress, or `paused`.
- **Durable payload:** status, summary, solutions, `placeholders`, `unexplainedMilitaryDelta2x`, and host-turn functional targets. Do **not** persist solver `diagnostics` (action catalogs / ship-build combo lists and band residual belong on the wire and in memory only).
