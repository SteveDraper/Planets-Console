...
- **Compute:** Issue #34 registers baseline ensure with the **compute orchestrator** and an empty `ENSURE_DEPENDENCIES` (game-global floor). [#36](https://github.com/SteveDraper/Planets-Console/issues/36) adds the linear self-chain (`homeworld-locator@T` depends on `homeworld-locator@(T−1)`) when shell-turn evidence refine is in scope. Issue #34 implements **baseline-only** ensure: writes game-global candidates + floor evidence aggregate at the baseline turn (turn 1, or degraded earliest). It does **not** copy-forward empty aggregates through the shell turn -- that is #36.
