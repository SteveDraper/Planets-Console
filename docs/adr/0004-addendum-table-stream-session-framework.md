...
- Scores shell registry + production persist gate + lifecycle applicator (`tier_row_run_registry`, `PersistDecision` / `decide_scores_row_persist`, `apply_scores_row_lifecycle`) -- instances of the generic stream-row pattern, not parallel ownership stories. Ownership invariants (cancel memory, sole drain ledger, persist plan, wake, fleet cancel asymmetry): [ADR 0006](0006-table-stream-lifecycle-invariants.md).
