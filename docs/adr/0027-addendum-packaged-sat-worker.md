...
Not chosen: a long-lived solver daemon with a second IPC protocol. Process SAT pickles a SAT-session job wire (model proto + search budget) into `ProcessPoolExecutor`; keep that plane.

The worker entry (`api.compute.sat_worker_entry`) imports the SAT-session worker (`run_sat_search_session`) only. It does not import AppKit, FastAPI `create_app`, `server.process_host`, file storage, turn codecs, or scores orchestration / `RowRun`. Freeze dests still must not collect the **console data directory**; the worker reads a model session wire, not `storageRoot`.
