...
- Stream teardown **detaches** observers only -- it does not cancel in-flight singleton DAG work (or abort a pending `persist`) solely because the observer left ([#209](https://github.com/SteveDraper/Planets-Console/issues/209)); origin-set prune is [#240](https://github.com/SteveDraper/Planets-Console/issues/240). Scores **RowRun phase** (single owner: `tier_row_run_registry`) is the persist-admission source of truth: `REGISTERED` → `DETACHED` (stream drop; shell retained; persist ALLOW) or `CANCELLED` (cancel intent; persist DENY) → drop on retire. Live `REGISTERED` persist leaves the shell for peer bindings; stream finalize retires it. Late `DETACHED`/`CANCELLED` persist retires immediately after the decision. **CANCELLED-only** shells are FIFO-bounded (`MAX_CANCELLED_ROW_RUNS`, same capacity class as `MAX_STREAM_RESOLUTIONS` / prior cancel-fence bound): shell eviction keeps a compact run_id denial so `PersistDecision` still returns `DENY_CANCEL` (never-seen ids stay `REFUSE_UNKNOWN`). Scores **cancel** (not detach) applies one cancel intent under the scheduler lock: cancel token + `CANCELLED` phase + stream-resolution `CANCELED` (delivery silence only), then aborts the orchestrator scope outside that lock. Soft/hard stream resolutions remain FIFO-bounded (`MAX_STREAM_RESOLUTIONS`) and delivery-only -- they do not gate persist. `PersistDecision` reads phase (and shell-evicted cancel denial) from the RowRun owner (`DENY_CANCEL` / `ALLOW` / `REFUSE_UNKNOWN`).
