...
When scores `tier_solve` is opted into **process**, dispatch submits the storage-rebuild leaf (`run_scores_tier_solve_leaf` on `remote_run_steps`), not the in-process `run_step`. The job wire carries scope ids, `runId`, `storageRoot` (file backend / packaged console data directory), observation, and a catalog-free `PolicyLadderState` snapshot. The child rebuilds `InferenceProblem` from storage and returns `StepResult`, echoing `runId` onto continue / persist / waiting_deps payloads. The parent applies those payloads to `RowRun` (`apply_scores_tier_solve_step_result`) on persist and on the next `tier_solve` wire build -- the child never looks up the registry. Persist requires string `runId` on the result wire; it does not fall back to scope lookup. Evidence-closed skip sentinels set `orchestrationSkip` and complete on the orchestration plane -- they are not submitted to `ProcessPoolExecutor`, and the leaf raises if that flag still arrives.
