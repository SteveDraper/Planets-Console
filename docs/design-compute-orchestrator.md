...
**Scores instance.** `tier_row_run_registry` is the scores owner of shells + compact cancel admission. `PersistDecision` / `decide_scores_row_persist` is the only production persist gate (atomic `snapshot_persist_decision` under one registry lock maps `PersistAdmission` and shell phase into write/refuse **and** retire plan). Once that decision is taken, a later cancel does not revoke it for that persist attempt; production writers must not branch on admission or shell phase beside that decision. Scores applies the three lifecycle ops through one command: `apply_scores_row_lifecycle` in `scores/row_lifecycle.py` (`RowLifecycleOp` from `row_run_admission.py`). Scheduler abort_scope and stream-map pops stay on the scheduler plane.
