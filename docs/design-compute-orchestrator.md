...
Thread steps (scores tiers, production default) resolve per-row adapter state (`RowRun`) by `run_id` on the job wire; ladder progress stays adapter-owned between continuations. They must not block on cross-node ensure or call `ensure_fleet_export` in the worker. The production thread job wire is the tiny identity `{runId, gameId, perspective, turn, playerId}` -- storage-rebuild fields (`storageRoot`, ladder snapshot, observation, time limit) are assembled only when the effective backend is `process`.

When scores `tier_solve` is opted into **process**, dispatch submits the storage-rebuild leaf (`run_scores_tier_solve_leaf` on `remote_run_steps`), not the in-process `run_step`. The job wire carries scope ids, `runId`, `storageRoot` (file backend / packaged console data directory), observation, and a catalog-free `PolicyLadderState` snapshot. The child rebuilds `InferenceProblem` from storage and returns a pickle-safe snapshot (echoed `runId` plus catalog-free ladder state). It does not choose continue/persist/waiting_deps. The parent maps that snapshot through `map_scores_tier_solve_remote_result` (`map_remote_step_results` on registration) after unpickle and before `coerce_step_result`: apply onto `RowRun` by echoed `runId`, then the canonical `outcome_after_ladder_complete` / `tier_job_outcome_to_step_result` mapper (soft-defer and fleet-torp diagnostics from the live `RowRun`). Persist requires string `runId` on the result wire; it does not fall back to scope lookup. Evidence-closed skip sentinels set `orchestrationSkip` and complete on the orchestration plane -- they are not submitted to `ProcessPoolExecutor`, and the leaf raises if that flag still arrives.
