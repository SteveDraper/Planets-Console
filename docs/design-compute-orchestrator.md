...
| cancel | drop immediately (no cancel phase) | `CANCEL_DENY` (scope-keyed compact memory) | `CANCELED` + drain closed via `stream_drain.seal_canceled` | cancel token, then `abort_scope` outside scheduler lock. Stream `reschedule_row` that cancelled a run must schedule a replacement (`force_fresh` submit) while the stream still includes the player -- cached-complete admission must not skip replace of the aborted node. |
