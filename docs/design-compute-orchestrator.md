...
- Stream teardown **detaches** observers only -- it does not cancel in-flight singleton DAG work (or abort a pending `persist`) solely because the observer left ([#209](https://github.com/SteveDraper/Planets-Console/issues/209)); origin-set prune is [#240](https://github.com/SteveDraper/Planets-Console/issues/240). Scores **RowRun phase** (single owner: `tier_row_run_registry`) is the persist-admission source of truth: `REGISTERED` → `DETACHED` (stream drop; shell retained; persist ALLOW) or `CANCELLED` (cancel intent; compact `run_id` admission only; persist DENY) → drop on retire. Live `REGISTERED` persist leaves the shell for peer bindings; stream finalize retires it. Late `DETACHED` persist retires the shell after the decision; `CANCELLED` deny retires compact admission memory. **Cancelled admission** is scope-keyed (at most one outstanding cancelled `run_id` per scores scope): no cancelled RowRun shell is retained, and a later `REGISTERED` for that scope supersedes the prior cancel. `REFUSE_UNKNOWN` (never-seen / retired / superseded cancel) and `DENY_CANCEL` both mean no durable write; persist policy treats both as silent no-ops (never raise). Scores **cancel** (not detach) applies one command (`apply_scores_row_cancel`): compact `CANCELLED` admission + stream-resolution `CANCELED` (delivery silence) + cancel token, then aborts the orchestrator scope outside the scheduler lock. Soft/hard stream resolutions remain FIFO-bounded (`MAX_STREAM_RESOLUTIONS`) and delivery-only -- they do not gate persist. `PersistDecision` is the only production persist gate (`DENY_CANCEL` / `ALLOW` / `REFUSE_UNKNOWN`).
