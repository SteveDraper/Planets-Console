...
| **process** | `ProcessPoolExecutor`; strongest isolation | prior-mining extraction (existing). Scores `tier_solve` **declares** `thread` (production default). Occupancy / tests opt into process SAT via `resolve_scores_tier_solve_backend` (`api.scores_tier_solve_backend=process` or `PLANETS_CONSOLE_SCORES_TIER_SOLVE_BACKEND=process`). The DAG step stays on `thread` (`run_scores_tier_solve` / `RowRun`); the parent Solve seam submits one SAT search session per near-best / exact / band pass to `run_sat_search_session` (CP-SAT model proto + search budget / `max_solutions` / cancel; assignments mapped back through the live catalog). The process payload is that session, not a ladder snapshot, and does not rebuild the turn from `storageRoot`. Frozen and unpackaged processes share that opt-in. A packaged process host must not `ProcessPoolExecutor` the windowed GUI executable: `multiprocessing.set_executable` points at the sibling `sat_worker` binary collected next to the process host, and spawn preparation omits the process-host `__main__` so the child does not reload `process_host_entry.py` ([ADR 0027 addendum](adr/0027-addendum-packaged-sat-worker.md)). Process SAT is still not the production default. |
...
Thread steps (scores tiers, production default) resolve per-row adapter state (`RowRun`) by `run_id` on the job wire; ladder progress stays adapter-owned between continuations. They must not block on cross-node ensure or call `ensure_fleet_export` in the worker. The production thread job wire is the tiny identity `{runId, gameId, perspective, turn, playerId}`. Process occupancy opt-in is a SAT-session worker (`run_sat_search_session`): one process job is a search session (loop of `Solve()` plus forbid-previous cuts, `StopSearch` / cancel in the child), not a storage-rebuild of the turn and catalog. The parent Solve seam builds the `CpModel` from `InferenceProblem` on the live `RowRun` catalog and submits that session; returned assignments are mapped to `InferenceSolution` on the parent. Streaming `on_admitted` stays parent-side (emit when a session result arrives). Nested session submits go straight to `ProcessPoolExecutor` so a thread worker waiting on SAT cannot deadlock the DAG work queue.

When scores `tier_solve` is opted into **process**, the orchestrator `run_step` stays the in-process ladder (`run_scores_tier_solve` / `RowRun`). Nested SAT sessions are not a DAG `remote_run_steps` leaf: the parent Solve seam submits `run_sat_search_session` through `submit_process_callable`. The child reads a model proto, search budget, `max_solutions`, and a cancel flag; it returns assignments and solver status. It does not load file storage, a turn codec, policy-ladder state, or `RowRun`, and it does not choose continue/persist/waiting_deps. Evidence-closed skip sentinels set `orchestrationSkip` and complete on the orchestration plane -- they are not submitted to `ProcessPoolExecutor`, and the session leaf raises if that flag still arrives.
...
| cancel | drop immediately (no cancel phase) | `CANCEL_DENY` (scope-keyed compact memory) | `CANCELED` + drain closed via `stream_drain.seal_canceled` | cancel token, then `abort_scope` outside scheduler lock. Stream `reschedule_row` that cancelled a run re-admits with `force_schedule` (`force_fresh` submit when scheduled) while the stream still includes the player -- cached-complete must not skip replace of the aborted node; Immediate/skip admission still applies. |
...
Self-chained analytics (`turn_delta=-1`) need contiguous stored turns from the ensure floor through the shell turn. `submit` and `ensure_scopes` call `prepare_dependency_chain_turns` **before** taking the orchestrator lock, using the login-backed `AnalyticQueryContext.ensure_turn` hook. `TurnAnalyticService` builds that context once for `(game_id, perspective, turn, username)` and every in-process compute caller uses it (table/map, fleet table-stream, scores table-stream, catalog export ensure, homeworld rematerialize). Stream adapters keep that leader ctx (scores may `replace` only the scheduler-bound scores bundle) and must not reconstruct from an analytic service bundle in a way that drops `ensure_turn`. Scores table-stream admit raises if the leader ctx is missing; scores-stream fleet warm submits that same factory ctx. That fetches missing TurnInfo via `ensure_turn` / loadturn. `plan_compute_dag` then walks only already-stored turns -- loadturn must not run under the orchestrator lock. If auto-fetch is unavailable or a fetch fails, Core raises `ValidationError` (422), not an uncaught `ValueError` 500.
