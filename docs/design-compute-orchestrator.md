...
- Stream teardown **detaches** observers only -- it does not cancel in-flight singleton DAG work (or abort a pending `persist`) solely because the observer left ([#209](https://github.com/SteveDraper/Planets-Console/issues/209)); origin-set prune is [#240](https://github.com/SteveDraper/Planets-Console/issues/240). Scores **RowRun shell phase** (`REGISTERED` / `DETACHED` only; single owner: `tier_row_run_registry`) is independent of **persist admission** (`PersistAdmission`: `ALLOW` | `CANCEL_DENY` | `ABSENT`). Shell path: `REGISTERED` → `DETACHED` (stream drop; shell retained; admission ALLOW) → drop on retire. Cancel intent drops the shell immediately and records compact cancelled admission (`CANCEL_DENY`) keyed by `run_id` / scores scope -- never a retained cancel shell phase. Live `REGISTERED` persist leaves the shell for peer bindings; stream finalize retires it. Late `DETACHED` persist retires the shell after the decision; cancel deny retires compact admission memory. **Cancelled admission** is scope-keyed (at most one outstanding cancelled `run_id` per scores scope): a later `REGISTERED` for that scope supersedes the prior cancel. `PersistDecision` is `ALLOW` or `REFUSE(should_retire)`: cancel admission maps to `REFUSE(should_retire=True)`; never-seen / retired / superseded cancel maps to `REFUSE(should_retire=False)`. Both refuse outcomes mean no durable write; persist policy treats them as silent no-ops (never raise) and retires only when `should_retire` is set. Scores **cancel** (not detach) applies one command (`apply_scores_row_cancel`): compact cancel admission + stream-resolution `CANCELED` (delivery silence) + cancel token, then aborts the orchestrator scope outside the scheduler lock. Soft/hard stream resolutions remain FIFO-bounded (`MAX_STREAM_RESOLUTIONS`) and delivery-only -- they do not gate persist. `PersistDecision` is the only production persist gate, mapped from `PersistAdmission`.
