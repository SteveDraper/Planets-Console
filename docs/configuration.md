...
| `scores_tier_solve_backend` | string | `thread` | Effective backend for scores `tier_solve` at dispatch/flush: `thread` (production default) or `process` (occupancy / test opt-in). The compute profile always declares `thread`; occupancy remaps at runtime. Frozen and unpackaged processes share this rule. Packaged `ProcessPoolExecutor` spawns the sibling `sat_worker` binary (not the windowed process host) and does not re-import the GUI `__main__` in those children. Env `PLANETS_CONSOLE_SCORES_TIER_SOLVE_BACKEND` overrides when set. |
