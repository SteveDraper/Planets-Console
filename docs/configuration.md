...
| `scores_tier_solve_backend` | string | `thread` | SAT search backend for scores inference: `thread` (production default, in-process `Solve()`) or `process` (occupancy / test opt-in). The compute profile always declares `thread` for the `tier_solve` DAG step so the policy ladder stays on the parent; process opt-in submits one SAT search session per pass to the `sat_worker` (`run_sat_search_session`), not a ladder snapshot. Frozen and unpackaged processes share this rule. Packaged `ProcessPoolExecutor` spawns the sibling `sat_worker` binary (not the windowed process host) and does not re-import the GUI `__main__` in those children. Env `PLANETS_CONSOLE_SCORES_TIER_SOLVE_BACKEND` overrides when set. |
...
That runs `scripts/bundle_console_package.py` / `scripts/console_package.spec`. Collect policy ships the prebuilt SPA (`packages/frontend/dist`), runtime `assets/analytics/`, and a one-line version sidecar -- not tests, `scripts/`, `docs/`, or frontend `src/`. The same freeze also collects a console `sat_worker` executable next to the process host (macOS `Contents/MacOS/sat_worker`, Windows `sat_worker.exe`) so packaged SAT workers are sibling binaries, not eight `.app` copies ([ADR 0027 addendum](adr/0027-addendum-packaged-sat-worker.md)). Identity: display name **Planets Console**, `CFBundleIdentifier` `com.github.stevedraper.planets-console`, `AppUserModelID` `SteveDraper.PlanetsConsole`, version from root `pyproject.toml` (`0.1.0`, not `0.1`). On macOS the bundler sets `MACOSX_DEPLOYMENT_TARGET=11`.

Occupancy probe (GIL overlap during `Solve()`, tmp-tree tiny file+JSON jobs, 1-thread / 8-thread `json.loads` / `deep_copy_value` / `FileStorageBackend` get+put of a synthetic ~775 KB / ~38k-node fleet-shaped document, `observationPersist` of that document through `run_fleet_observation_leg` + `FleetPersistencePolicy.persist`, optional `scoresSolve` of one real scores `tier_solve` `Solve()` when `SCORES_SOLVE_TREE` points at an existing game tree (read in place; not copied into the repo), and on a frozen macOS process host `appKitOn` GIL + large-document timings while `NSApplication.run` is on the main thread; no SPA settle, no Application Support store). Unpackaged baseline:
...
To time a real scores `tier_solve` against an existing store (game 683364 / viewpoint 1 / turn 27 by default):

```bash
make console_package_probe SCORES_SOLVE_TREE=./.data
make console_package_probe FROZEN=1 SCORES_SOLVE_TREE=./.data
```
...
CI workflow [`.github/workflows/console-package-release.yml`](../.github/workflows/console-package-release.yml) runs on tag glob `v*.*.*` and on `workflow_dispatch`; both fail unless the git ref name equals `v` plus root pyproject version. It does **not** use `on: release: published`. Freeze jobs use pinned runners `macos-15` and `windows-2025` (not `*-latest`): SPA build, bundler, wrapper, then `gh release create` (or reattach with `--clobber`) in the same run. Release title is **Planets Console `<version>`**. Uploaded files are unsigned.

Cut a release from a clean tree (do not create the GitHub Release in the UI):

```bash
make release              # revision bump, e.g. 0.1.0 -> 0.1.1
make release ARGS='--minor'
...
- **Launch contract (`test_process_host_launch.py`):** primary start waits for `GET /health` before opening `http://127.0.0.1:<port>/`; health timeout never opens the SPA and names the support log; successful Quit releases the lock then `os._exit`s (no interpreter teardown); packaged `storage_root` is the console data directory, not cwd `./.data`; `main()` shows the start-failure dialog and exits 1 (macOS osascript / Windows `MessageBoxW` contracts); missing SPA still writes `logs/process-host.log` under the data directory.
- **Occupancy probe (`test_packaged_runtime_probe.py`, `scripts/tests/test_console_package_probe.py`):** `--console-package-probe` writes JSON (`pythonProgressedDuringSolve`, file+JSON counts/wall, `largeDocument` 1-thread/8-thread json.loads/deep_copy/get/put, `observationPersist`, optional `scoresSolve` when `--scores-solve-tree` / `SCORES_SOLVE_TREE` is set, `appKitOn` skip-or-run, interpreter-pool error) to `--output` and does not start the SPA or use the console data directory; `make console_package_probe` / `FROZEN=1` are documented; dispatch-only `macos-15` workflow runs uv + frozen probes; ubuntu `make ci` does not freeze.
