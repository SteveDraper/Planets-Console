...
Default-path scores YAML (`tier_policy.yaml`, `prior_weights_*.yaml`) is loaded once per process so concurrent `tier_solve` workers do not re-parse it under the GIL. To re-check packaged SAT-only occupancy after a bundle: stop any running Planets Console first, then LaunchServices `open -n` of the `.app` with `--args --no-browser` (unpackaged: `python -m server.process_host --no-browser`). Stream fleet+scores from the measurement client only, for game **683364** / viewpoint **1** / turn **27**, and `sample` the process-host pid while `globalInFlight` is near 8 and the reconstructed mix is SAT-only (same method as `/tmp/issue478/live`). The flag keeps the SPA from opening a second fleet or scores stream on that process. CPU-while-full should sit materially above one core; worker stacks should not convoy in `yaml.safe_load` / prior-weights parse.
...
- **Launch contract (`test_process_host_launch.py`):** primary start waits for `GET /health` before opening `http://127.0.0.1:<port>/`; `--no-browser` skips that open, a second activation's reopen, and dock reopen; health timeout never opens the SPA and names the support log; successful Quit releases the lock then `os._exit`s (no interpreter teardown); packaged `storage_root` is the console data directory, not cwd `./.data`; `main()` shows the start-failure dialog and exits 1 (macOS osascript / Windows `MessageBoxW` contracts); missing SPA still writes `logs/process-host.log` under the data directory.
