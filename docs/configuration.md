...
| `scores_tier_solve_backend` | string | `thread` | Declared backend for scores `tier_solve`: `thread` (production default) or `process` (occupancy / test opt-in). Frozen processes stay on `thread` so this phase does not start `ProcessPoolExecutor` in a packaged `.app`. Env `PLANETS_CONSOLE_SCORES_TIER_SOLVE_BACKEND` overrides when set. |
