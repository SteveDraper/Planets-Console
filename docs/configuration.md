...
Occupancy probe (GIL overlap during `Solve()`, tmp-tree tiny file+JSON jobs, and 1-thread / 8-thread `json.loads` / `deep_copy_value` / `FileStorageBackend` get+put of a synthetic ~775 KB / ~38k-node fleet-shaped document; no SPA settle, no Application Support store). Unpackaged baseline:
