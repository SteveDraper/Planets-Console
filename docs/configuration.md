...
| `homeworld_locator.origin_distance_evidence_lambda` | float | `0.8` | Temporal blend weight λ for soft origin-distance evidence in layout-prior cost: running `E = (E + λ e) / (1 + λ)` over turn-ordered observation means. Valid range `(0, 1]`. |
...
api:
  ...
  homeworld_locator:
    ...
    origin_distance_evidence_lambda: 0.8
