...
| **Homeworld region selection** | Sidebar control (Cartography sticky preference): preset **Pinned / Unpinned / Selected** (default Selected). Pinned/Unpinned derive the visible set from overlay facts; Selected + `null` stored indexes = all sectors; direct tile toggles force Selected and write an explicit set |
