...
| Invalidation | Fleet final persist at `(N, player)` must invalidate / wake `homeworld@N` when that node already ran or is waiting (same epoch/invalidation pattern as scores↔fleet) |
