...
| FE display | **Homeworld region selection** (#283): preset `pinned` \| `unpinned` \| `selected` (default `selected`). Pinned/Unpinned derive the outline set from overlay facts at read time; Selected uses stored indexes with `null` = **all** sectors (never-materialized default). Switching from Pinned/Unpinned to Selected materializes the effective derived set. **Show overlays** checkbox (default on) toggles 81/162 envelope disks for **selected** sectors only. Replaces #35's `off` \| `un-pinned` \| `pinned` \| `all` display mode |
...
| **Homeworld region selection** | Sidebar control (Cartography sticky preference): preset **Pinned / Unpinned / Selected** (default Selected). Pinned/Unpinned derive the visible set from overlay facts; Selected + `null` stored indexes = all sectors (never-materialized default); Pinned/Unpinned → Selected materializes the effective set; direct tile toggles force Selected and write an explicit set |
