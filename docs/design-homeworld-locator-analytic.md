...
| Co-sector cull | Once a sector has a **definite** homeworld, drop other inferred candidates in that same angular sector (possibles and evidence-promoted orphan definites). Prefer slot-anchored over orphan when choosing which inferred definite to keep; never cull **location-asserted** rows (durable location keys / `location_asserted`; ownership-only `asserted_cue` does not protect) |
...
| Persistence | Turn-scoped on the **homeworld evidence aggregate** (durable sector owner sets + provenances through *T*); materialize projects onto sector overlays / candidate bind helpers. Stamp ``evidenceAlgorithmVersion`` (`HOMEWORLD_EVIDENCE_ALGORITHM_VERSION`, currently **5**; absent/0 = pre-version). Satisfaction / ensure refuse stale versions so the self-chain rewalks from the baseline floor. **Baseline floor write** runs one evidence-refine accumulate on the baseline turn (preferred-candidate / nearby ownership) because the DAG does not ensure turn 1 as a refine node -- otherwise the viewpoint definite HW would not pin its sector at T1. Floor *algorithm* rewrite is owned solely by ``ensure_evidence_floor_algorithm_current`` (clears sticky ownership before re-accumulate, then stamps the current version) |
| Location candidates | Never create orphans; never flip `confidence_tier` from ownership alone |
| Candidate `perspective` | Optional bind when a sector has a **unique projected** possible owner (max-strength + drop slots uniquely settled on another sector -- the same test as overlay ``isPinned``). Bind iterates to fixpoint (cap = player count) so a bind-created definite pin can settle the next sector in the same pass overlay already uses. Orphans in that sector may become slot-anchored for layout-prior. Ambiguous projected sets leave orphans unbound. Prose always says **homeworld owner**, not perspective |
...
**Materialize order:** after location promote + co-sector cull + definite-neighborhood cull; **before** layout prior. Ownership apply updates sector owner sets. Unique-owner orphan bind uses the same projected uniqueness as overlay ``isPinned`` (shared ``project_sector_owner_sets_with_location_pins``) and iterates to fixpoint after location derive so a newly bound definite pin can settle the next sector in the same pass. Layout prior still keys off candidate tiers / pins.

**Wire / UI (minimal for #269):** sector `regionOverlays` carry **display-projected** possible-owner facts (slot ids, roster labels, provenance kind counts, winning strength). Projection (overlay emit and unique-owner bind -- not durable trim): max-strength contenders per ADR 0010, then drop slots uniquely settled strong/asserted (or definite slot-anchored location pin) on another sector so assert changes cannot leave sticky holes. **``isPinned``** is that projected set having exactly one member -- not “a slot-anchored candidate lies in the wedge.” SPA hover shows a single owner when `|set|=1` (`definite` when winning strength is strong, `inferred` when weak, `asserted` when asserted), else **ambiguous** plus the possible owners when `|set|>1`. Accordion titles follow the same projected set (not raw `playerLabel` over multi-member evidence). Pinned ``playerLabel`` is the unique projected owner's roster identity.
...
| Pinned sectors | Layout-prior: the **definite** planet is the fixed set member (no most-probable label). Distinct from overlay ``isPinned``, which is unique projected owner |
...
3. **Map paint** -- outlines for selected sectors only; envelopes when checkbox on and sector selected; MapGraph / style tests.
...
| 2026-08-12 | Overlay ``isPinned`` / unique-owner orphan bind share one test: unique projected possible owner after settled-sector elimination (not slot-anchored presence) |
| 2026-08-12 | Unique-owner orphan bind iterates to the same uniqueness fixpoint overlay projection sees (bind-created definite pins settle the next sector in one materialize) |
| 2026-08-14 | Baseline floor write runs one evidence-refine accumulate (preferred-candidate / nearby ownership) so the viewpoint definite HW pins its sector at turn 1; DAG still skips turn 1 as a refine node. Bump ``HOMEWORLD_EVIDENCE_ALGORITHM_VERSION`` to 5 |
