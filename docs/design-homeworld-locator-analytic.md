...
**Materialize order:** after location promote + co-sector cull + definite-neighborhood cull; **before** layout prior. Ownership apply updates sector owner sets. Unique-owner orphan bind uses the same projected uniqueness as overlay ``isPinned`` and runs after location derive so definite pins can settle other sectors. Layout prior still keys off candidate tiers / pins.
