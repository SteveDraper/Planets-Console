...
| Solver implementations | `#36` capped product + fixed mid stand-in remains as `EnumeratingLayoutPriorSolver` (regression / fixtures / emergency config). Production default becomes the SA solver after it lands. YAML exposes budget ms + solver selector only. SA cooling, moves, and proposal bias live in code and may be **problem-size adaptive** (e.g. function of choice-sector count, possibles, planet count, and/or layout category); coefficients are chosen via fixtures, not YAML tables. Modules: `layout_prior.py` (facade), `layout_prior_problem.py`, `layout_prior_cost.py`, `layout_prior_solver.py`, `layout_prior_stop_gate.py`, `layout_prior_enumerate.py`. |
