...
| **Homeworld envelope overlay toggle** | **Show overlays** checkbox (above Region selection when that control is visible); when on, paint 81/162 LY disks for selected sectors only (planet envelopes for pinned/asserted rows are a follow-on) |
