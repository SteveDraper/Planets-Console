...
**Predicates:** `hasDisplayableMapData`, `deriveTurnEnsureLoadingView`, and `deriveMapShellView` in `mapDisplayRetention.ts` (unit-tested separately). Map retention during reload is expressed solely via `retainDuringLoad` on `deriveMapShellView` (not a separate turn-loading suppress flag).
