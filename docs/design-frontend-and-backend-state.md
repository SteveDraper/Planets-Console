...
| **Scores inference revision** | `packages/frontend/src/stores/scoresInferenceRevision.ts` | Monotonic per-scope revision bumped when the scores inference NDJSON stream emits evidence that can change fleet composition: every `complete` event; `solution` events when held `solutions` change or first-class `fleetTorpInputStatus` changes vs the last known value for that `(scope, playerId)`. Per-player dedup memory is cleared on stream reconnect for the scope. **Fleet table** no longer includes this counter in its query key ([#169](https://github.com/SteveDraper/Planets-Console/issues/169)); per-player fleet tile updates come from the fleet table NDJSON stream instead. Future fleet **map** queries may still need a revision or stream hook ([#128](https://github.com/SteveDraper/Planets-Console/issues/128)). |
