...
| **Connections map params** | `packages/frontend/src/analytics/connections/connectionsMapParamsStore.ts` | Ephemeral warp/flare knobs for the Connections map GET. **Not persisted** -- reset on full page reload. Shared by the sidebar tile, `queryParams.appendMap`, and the map-registry query key. |
