...
| `['analytic', 'fleet', 'table', <scope>, scoresInferenceRevision]` | **Fleet** tabular analytic | Same scope object as other table analytics, plus `scoresInferenceRevision` from `stores/scoresInferenceRevision.ts` so fleet rematerializes when scores inference evidence changes. Future fleet map queries should include the same revision ([#128](https://github.com/SteveDraper/Planets-Console/issues/128)). |
