...
| **Visibility preferences** | `packages/frontend/src/stores/visibilityPreferences.ts` | Persisted per-kind toggles and fill colors for the Visibility analytic (`planets-console-visibility-preferences`; default all kinds on with distinct colors). |
| **Display preferences** | `packages/frontend/src/stores/displayPreferences.ts` | Player/sector list label modes (persisted). |
| **Scores inference revision** | `packages/frontend/src/stores/scoresInferenceRevision.ts` | Monotonic per-scope revision bumped when the scores inference NDJSON stream emits evidence that can change fleet composition: every `complete` event; `solution` events when held `solutions` change or first-class `fleetTorpInputStatus` changes vs the last known value for that `(scope, playerId)`. Per-player dedup memory is cleared on stream reconnect for the scope. **Fleet table** and **fleet map layer** both consume the **fleet stream** ([ADR 0011](adr/0011-fleet-stream-behind-table-and-map.md), [#128](https://github.com/SteveDraper/Planets-Console/issues/128)); map does not use this revision counter or a separate map GET. |
| **Connections map params** | `packages/frontend/src/analytics/connections/connectionsMapParamsStore.ts` | Ephemeral warp/flare knobs for the Connections map GET. **Not persisted** -- reset on full page reload. Shared by the sidebar tile, the Connections map appender in `shellAnalyticQueryParams.ts`, and the map-registry query key. |
...
| `['analytic', 'fleet', 'table', <scope>]` | **Fleet** tabular analytic (REST bootstrap) | Initial `componentCatalog`, player names, and first materialized records. Per-player refinement and ledger updates arrive on the fleet table NDJSON stream (`useFleetTableStream`) for **visible** players from fleet player visibility ([#169](https://github.com/SteveDraper/Planets-Console/issues/169)). Stream connect shares the same per-player ensure/materialization path as export ensure orchestration ([#109](https://github.com/SteveDraper/Planets-Console/issues/109)); the SPA does not duplicate ensure UI -- it consumes streamed `ledger_updated` / `record_refined` events. |
...
**Enabled analytic ids** and **Stellar Cartography layer toggles** live in persisted Zustand stores (see section 1), not in `App.tsx` local state. **Connections map params** live in an ephemeral Zustand store under `analytics/connections/` -- also not `App.tsx` local state, and **not persisted**.
