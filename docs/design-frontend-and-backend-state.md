...
| **Scores inference revision** | `packages/frontend/src/stores/scoresInferenceRevision.ts` | Monotonic per-scope revision bumped when the scores inference NDJSON stream emits `solution` or `complete` with first-class `fleetTorpInputStatus` that changes vs the last known value for that scope. Downstream analytics (e.g. fleet table) include this counter in TanStack Query keys so they refetch without scores UI calling `invalidateQueries` for other analytics. |
