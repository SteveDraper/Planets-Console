...
**Predicates:** `hasDisplayableMapData`, `deriveTurnEnsureLoadingView`, and `deriveMapShellView` in `mapDisplayRetention.ts` (unit-tested separately). Map retention during reload is expressed via `mapFrameSource: 'live' | 'retained' | 'none'` on `deriveMapShellView`; a retained frame suppresses turn-ensure and deferred loading UI while the prior map stays visible.
