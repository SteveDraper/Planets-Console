...
Product **`status`**, leftover (`unexplainedMilitaryDelta2x`), and **`placeholders`** are siblings of **`$.solutions`**. Residual / `no_exact_solution` rows expose leftover plus `placeholders` (unknown military ship hull `-1` and observation-derived generic freighter hull `0` when remainders are positive). Admission-skip rows expose per-reason `status` and empty `placeholders` with leftover omitted. Do not collapse skip / residual / no-solution into the five generic `searchStatus` values. Stale `$.solution.ships` / `$.solution.diagnostics` paths are gone.
