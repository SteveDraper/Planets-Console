...
- **MCP hatch-read** (when the adapter ships): Core tests own ensure-final / `needs_ensure` / `in_progress` vs `ctx.query`. `mcp_adapter` tests assert tool envelope equality to that Core function (fixture + **connections**) and must not re-golden export trees ([ADR 0025](adr/0025-mcp-testing-and-contract.md)).
