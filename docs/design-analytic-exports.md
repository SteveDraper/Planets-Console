...
4. **Storage floor** -- if a dependency turn *at or above* the ensure floor is not stored for the **perspective**, probe reports `turn_not_stored` (root **unavailable**), not a neutral baseline. Compute DAG planning (`submit` / `ensure_scopes`) auto-fetches those turns when a login-backed `ensure_turn` hook is on the query context; otherwise it fails with `ValidationError`.
...
Product **`status`**, leftover (`unexplainedMilitaryDelta2x` or tagged **unknown-loss leftover**), **`placeholders`**, and **`latticeSignatures`** are siblings of **`$.solutions`**. Residual / `no_exact_solution` rows expose leftover plus `placeholders` (unknown military ship hull `-1` and observation-derived generic freighter hull `0` when remainders are positive). `uncharacterized_roster` rows expose tagged leftover, **placeholder departure**s, and lattice signatures; a bound leftover is not a point `unexplainedMilitaryDelta2x`. Admission-skip rows expose per-reason `status` and empty `placeholders` with leftover omitted. Do not collapse skip / residual / no-solution / roster into the five generic `searchStatus` values. Stale `$.solution.ships` / `$.solution.diagnostics` paths are gone.
...
| `$.leftover` | requires `player_id` (tagged leftover, including unknown-loss bound) |
| `$.latticeSignatures.*` | requires `player_id` (uncharacterized roster lattice alternatives) |
...
Production wiring: **#133**. Stream diagnostics and SPA UX: **#158** (see [design-military-score-build-inference-implementation.md](design-military-score-build-inference-implementation.md) section 8.8.5). Catalog filter widens for collision twins use step-local `includeComponentIds` (#226), not a cancelled #78 resolve-time overlay.
...
- `list_analytic_exports` -- optional `analytic_id`; `detail=summary|full`. Omit id defaults to **MCP export catalog summary**; named id defaults to full catalog (value schema, path-prefix rules, ordering semantics, ensure dependencies). Omit-id + `detail=full` is refused ([ADR 0024](adr/0024-mcp-result-size-and-query-cost.md)).
- `query_analytic_export(analytic_id, scope, paths[])` -- same result envelope as Core **analytic export hatch-read** (`AnalyticQueryContext.hatch_read`: ensure-final materialize), not `ctx.query` (which admits ensure). Does not admit new **analytic export ensure**. Materializes only persisted / ensure-final. Otherwise `unavailable` with `needs_ensure` or `in_progress`. Agent polls until `ok`. Successful payloads are capped by the **MCP hatch result budget** ([ADR 0024](adr/0024-mcp-result-size-and-query-cost.md)). JSONPath stays the Core subset; top-K is a batched index list, not a slice. Hatch parity tests: [ADR 0025](adr/0025-mcp-testing-and-contract.md).
...
No second implementation path. Auth and identity for the adapter: [ADR 0014](adr/0014-mcp-login-identity-and-visibility.md) (**MCP login identity**, **MCP visibility ceiling**). **Viewpoint eligibility** of the named slot: [ADR 0019](adr/0019-viewpoint-eligibility-in-core.md). Scope on query and ensure is the same per-call **shell context** as other turn-scoped MCP tools ([ADR 0018](adr/0018-mcp-shell-context-binding.md)); list is login-only. First-slice human-parity (TurnInfo is **MCP TurnInfo fallback**, not the advisor API): [ADR 0016](adr/0016-mcp-turninfo-fallback-and-disk-proximity.md). Testing and contract (no OpenAPI; Core owns hatch-read gates; adapter owns envelope equality): [ADR 0025](adr/0025-mcp-testing-and-contract.md). Design index: [design-mcp.md](design-mcp.md).
...
- **Analytic export hatch-read** ([#333](https://github.com/SteveDraper/Planets-Console/issues/333)): Core tests own ensure-final / `needs_ensure` / `in_progress` vs `ctx.query` (fixture alpha/beta gates; **connections** empty-catalog equality). `mcp_adapter` tests (when the adapter ships) assert tool envelope equality to `AnalyticQueryContext.hatch_read` (fixture + **connections**) and must not re-golden export trees ([ADR 0025](adr/0025-mcp-testing-and-contract.md)).
