...
- **Document LRU:** The file backend may retain a bounded LRU of admitted breakpoint JSON, process-wide per resolved `storage_root`. Values stay `JSONValue`. Turn RST (`games/*/*/turns/*`) and credentials (`credentials/accounts/*`) are excluded so they are not kept beside the process TurnInfo LRU. Analytic documents (including `…/turns/*/analytics/*`) are admitted. Successful disk `put` write-throughs the merged document; `get` still `deep_copy_value` on the way out; delete/prune drops the entry. Filesystem `list()` prefixes are a separate bounded map; put/delete of a child invalidates ancestor listings rather than patching name lists. External writers and a new process see a cold LRU.
...
- Bounded JSON-document LRU: second get of an admitted analytic document is a cache hit; put write-through; suffix put caches the merged document; delete drops the entry; turn RST and credentials are not retained; eviction past maxsize; two backends on the same root share hits and distinct roots do not; filesystem `list` of an analytics prefix is cached and invalidated on child put/delete.
