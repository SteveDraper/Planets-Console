...
Regenerate SPA OpenAPI types if BFF query or response models change: `make generate` (or `cd packages/frontend && npm run generate:api`). Updates monolithic `schema.ts` and per-slice `schema-<slice>.ts` during parity; see [ADR 0003](adr/0003-frontend-bff-contract-codegen.md).
