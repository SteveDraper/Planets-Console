...
`TurnAnalyticService` loads `TurnInfo`, builds `TurnAnalyticsOptions`, and routes registered table/map compute through the [**compute orchestrator**](design-compute-orchestrator.md) ([#202](https://github.com/SteveDraper/Planets-Console/issues/202)): cache miss / invalidation submits batch `ComputeRequest`s (`ensure_scopes`, `interactive_ensure`); cache hits short-circuit on the `inline` backend via `PersistencePolicy.is_satisfied`. Analytics without a compute profile (and scores REST, whose table is a TurnInfo projection) still dispatch `compute()` directly. The handler then shapes the table/map wire from persisted artifacts or `TurnInfo`.
