...
**Host-turn persist gate:** `FleetPersistencePolicy.persist` re-resolves provenance after scores refine. If `turnEvidenceAtN` is still false, it raises `FleetScoresEvidenceOpenError` (HTTP 409) instead of writing a non-final ledger and completing the fleet node. Completing with open scores evidence unlocked dependents and parked the chain with no automatic rematerialization after #233 removed sync gap-fill spin. The orchestrator parks that fleet node on `waiting_deps` and force_freshes same-turn scores so a durable close rematerializes fleet (stream-only reschedule cannot wake historical/background DAG nodes). Scores `is_satisfied` / skip-complete uses the same materialization probe as fleet (no ensure-ephemeral), so cheap ensure terminals cannot unlock fleet without disk evidence.
