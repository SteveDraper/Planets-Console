...
| **Chain** | Materialize turn `T` for player P from P's ledger at `T-1` + evidence on turn `T` for P only. When P is eliminated on `T`, do not read `T-1` or a baseline; the ledger is empty |
| **Baseline** | Turn 1: empty ledger or sightings-only seed per **Player**. Skipped when the player is eliminated on the turn being materialized |
| **Provenance** | Per ledger: `(turnEvidenceAtN, priorLedgerAtNMinus1)`. Both `true`, plus current materialization version and a matching **fleet evidence generation** (or a turn before the mark's `appliesFromTurn`) --> persisted ledger is **final** for ensure/probe. Otherwise the scope needs further ensure work. A non-final file stays readable. **Exception:** a player eliminated on this turn (`is_eliminated_at_turn`) is final via `eliminatedAtTurn` with both input flags false and no ship records |
...
**Re-queue:** orchestrator fleet nodes already discard on epoch mismatch (`generation_at_submit` → re-queue), including the persist-then-complete window. Scores evidence updates that clear fleet ledgers call `reschedule_fleet_table_player` when a fleet table stream is open for that turn; otherwise they `force_fresh`-submit the fleet compute scope (`wake_fleet_scope`) so a hollow terminal (durable gone, DAG still `complete`) is replaced even after the user has left that turn. Export `ensure_fleet_export` catches `FleetGapFillEpochInvalidated` and returns unsatisfied so a later ensure / DAG submit can complete once evidence is stable. Ensure-final still requires `has_final_ledger` (both provenance flags, or `eliminatedAtTurn` on an empty ledger); non-final ledgers, including the eliminated observation write, must not short-circuit ensure.

**Host-turn persist gate:** `FleetPersistencePolicy.persist` re-resolves provenance after scores refine. If `turnEvidenceAtN` is still false, it raises `FleetScoresEvidenceOpenError` (HTTP 409, a `PersistDeferredError`) carrying a `PersistDependencyRecovery` that force_freshes same-turn scores `tier_solve`, instead of writing a non-final ledger and completing the fleet node. **Eliminated player:** when `is_eliminated_at_turn` is true for that player's row at the scope turn (`status == ELIMINATED` and `turn >= statusturn`, including the elimination turn), the ensure walk and DAG drop `fleet` at `turn - 1`, and materialization does not load a prior ledger or baseline (chain gap-fill included, and the skip-missing-prefix seed). The observation job wire is the `eliminatedAtTurn` flag only. Observation persist writes an empty ledger with every closure flag false, so `has_final_ledger` stays false and `on_ledger_persisted` does not fire. Finalization persist writes that empty ledger with `eliminatedAtTurn` and does not raise `FleetScoresEvidenceOpenError`. That flag is the finality rule; it does not mean `turnEvidenceAtN` or `priorLedgerAtNMinus1` arrived. Turns before `statusturn`, and an active player with `statusturn` set (slot takeover), keep the prior-fleet edge and this scores gate. `username == "dead"` or `statusturn` alone is not elimination. Completing with open scores evidence unlocked dependents and left the chain with a non-final ledger and no automatic rematerialization after #233 removed sync gap-fill spin. The orchestrator handles `PersistDeferredError` generically: demotes the node to `waiting_deps` (not soft `parked`) and submits the declared dependency so a durable close rematerializes fleet (stream-only reschedule cannot wake historical/background DAG nodes). Scores `is_satisfied` / skip-complete uses the same materialization probe as fleet (no ensure-ephemeral). Cheap ImmediateRowAdmission ensure admits persist fallback-complete disk rows (`no_prior_turn` / `player_not_found`) with `put_row(..., notify=False)` so that probe can close without relying on ephemeral-only state and without firing fleet invalidation mid gap-fill.
...
**Scores invalidation coupling ([#182](https://github.com/SteveDraper/Planets-Console/issues/182), [#184](https://github.com/SteveDraper/Planets-Console/issues/184), grill [#204](https://github.com/SteveDraper/Planets-Console/issues/204)):** when a player's ensure-final `fleet@(N - 1)` ledger is persisted, the `on_ledger_persisted` side channel may drop scores inference for that player at host turn *N* and reschedule an open table-stream row. This is **not** how first-pass DAG dataflow works (`waiting_deps` + `DependencyOutputs`). Under #204: fire the side channel on each fleet node **final** complete (no coordinator end-of-chain batch). **Own-DAG elision:** if `scores@N` is already a non-terminal node on the same orchestrator DAG that depends on that `fleet@(N-1)` scope, do **not** wipe or reschedule — end-batch notify after a multi-turn unwind was worse (it invalidated work the DAG just finished). Keep the channel for ambient/external consumers (e.g. open scores stream that completed with pending prior fleet). Roster-level `on_snapshot_persisted` remains legacy-only for explicit `put_snapshot` / migration callers.
...
Same-turn **scores** is not an ENSURE edge on fleet observation. Finalization waits on scores via `PersistDeferredError` (orchestrator `waiting_deps` + `force_fresh` on scores `tier_solve`). An eliminated player at the scope turn drops the `turn_delta=-1` edge for that scope (`fleet_ensure_dependencies_for`) and does not wait on scores. That observation job wire is the `eliminatedAtTurn` flag only; observation writes an empty non-final ledger and continues; finalization writes the empty final ledger (`eliminatedAtTurn`, no scores refine). Living players follow the usual compute profile: `observation_leg` (declared interpreter, remapped to thread in a frozen console package -- [#451](https://github.com/SteveDraper/Planets-Console/issues/451); non-final host ledger + continue; no ship-observation ingest) then `finalization_leg` (inline, scores refine then observation ingest + final persist).
...
Export tree mirrors ledger: per-player records, discrepancies, `meta.searchStatus` when scores materialization incomplete for the scoped `player_id`. `is_ensure_satisfied` for `fleet@N` requires a per-player ledger that `has_final_ledger` accepts: provenance `(true, true)`, or `eliminatedAtTurn` on an empty ledger. A stored non-final document is not enough. See [design-analytic-exports.md](design-analytic-exports.md) and [ADR 0004](adr/0004-fleet-per-player-persistence-and-ensure-provenance.md).
...
Optional `motion` is Core-attached on table/stream wire only when the record has a known ship id present in the current-turn `TurnInfo.ships` that is underway (`targetx`/`targety` ≠ position) with usable warp and a resolvable heading (vector to the waypoint; `Ship.heading` only if the waypoint coincides with position). Shape:
...
- **Fleet heading trail**s: for each visible **active** ship with wire `motion`, draw player-colored rays from the location-ring origin along `heading`, length `travelLyPerTurn` per segment (forward legs clamp to `trailStop` or, when warp ≥ 2, a planet/well stop from base-map centers + shipped `normalWellCells` -- exact planet on the segment, or end-of-turn in a well cell; backward legs likewise, and are omitted when the origin is already on a planet / in a well). **W1** skips SPA planet/well path clamps (host well-pull exemption) but still honors `trailStop`. Sidebar **Trail** control `N` ∈ 0..5 (persisted): `0` = current-turn segment only (default); `N > 0` adds forward `+1..N` and backward `-1..-N` with opacity `f(|turnOffset|)` (current highest). Backward rays are estimate-only (reverse of current heading/speed). Performing **hyperjump** (`motion.hyperjump`): dotted current-turn ray to the HYP landing only -- never multi-turn extend, and no mid-path planet/well clamps. Omit all heading trails (and disable the Trail control) when the shell selected turn is **future** (`selectedTurn` > hosted turn) -- no movement simulation beyond hosted data. [#290](https://github.com/SteveDraper/Planets-Console/issues/290)
...
| **Fleet gap-fill coordinator** ([#161](https://github.com/SteveDraper/Planets-Console/issues/161), scope [#179](https://github.com/SteveDraper/Planets-Console/issues/179)) | Historical; **deleted by [#204](https://github.com/SteveDraper/Planets-Console/issues/204)** — orchestrator DAG + singleflight replaces it (see section 5.1) |
