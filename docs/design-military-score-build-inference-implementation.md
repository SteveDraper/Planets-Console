...
14. **Worthwhile remainder bound (locked #399; implementation later on #394):** in-regime ship-first overshoot is a hard cap, not score band. Exact-turn p90 of positive-stock units and field count, equal-split decay to leftover 2x, **remainder-bound turn mixture** (`n_total/(n_total+20)`, same-race isotonic leftover vs T, hold last knot), **observed-stock floor** from current-turn visible owner fields (exact per-field decay). CP-SAT `cap_2x = floor(bound)`. Contract: design §3.9.
