...
| **Inference moderate residual** / **mine-score residual** / `no_exact_solution` (stream/SPA chrome shipped, #367; abort classifier shipped, #368; placeholders shipped #369) | Distinct statuses (`moderate_residual` / `mine_score_residual` / `no_exact_solution`); leftover + `placeholders[]` on persist / `complete`; not an inexact action list. Design §3.6 / §3.8. Cheap-unsat rows take **inference expensive-tier abort** when the hopeless classifier fires. |
