...
17. **Pinned vs uncharacterized departure (design #483; pin determination shipped #486; SAT admission shipped #487):** `count_lattice.py` owns **count-lattice signal** and **departure pin**. `military_sat_admission.py` refuses `tier_solve` military catalog when a leave-signal on a military-moving class is unpinned (in-memory `military_sat_refused`; not `no_exact_solution`, not persisted). Unique-fill persist is not a pin and does not open extra-warship capacity. Uncharacterized-roster emit, persist, fleet retire, and chrome remain follow-ons #488–#491. Combat/explosion extras: #485. Contract: design §3.12.
