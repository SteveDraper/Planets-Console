...
|------|---------|------------|----------------|
| **Inference solution count indicator** | Outlined badge with **N** = rows currently held in **inference merged top-K**. Exact is green; complete **mine-score residual** with **ship-first near-solution**s is the same badge in blue. While search is in flight and **N = 0**, show a **dashed-border badge with 0** (same count-badge chrome, not a separate hourglass icon). When **N > 0** on the exact path, use a solid border; **N** rises toward K while search continues until `isComplete`. Overshooting ship-first lists do not appear mid-search. In-progress search adds an animation affordance on the badge. | summarize top solution when **N > 0** (blue badge: N first, leftover second); show that inference is still running when **N = 0** | open modal with ranked held solutions when **N > 0** (exact: grows while streaming until plateau at K; residual list arrives on `complete`). Exact path: no modal until the first held leftover-0 arrives |
...
| Residual / skip chrome (#360 persist shipped in #366; stream/SPA chrome shipped in #367; ship-first list persist #413 / chrome shipped #415) | Distinct from the single red X: muted skip (no modal); empty-list `moderate_residual` vs `mine_score_residual` markers + leftover; complete **mine-score residual** with **N > 0** uses the blue count badge (not hexagon). `no_exact_solution` keeps the red X. Design §3.8. | empty-list: status + leftover; blue N: count first, leftover second | skip: no modal; residual / complete-failure: existing detail, no new modal |
...
- Opens when **N > 0** and row status is `success`, `paused`, or complete `mine_score_residual` with **ship-first near-solution**s; exact path live-updates while search runs (residual list arrives on `complete` only).
...
- All held top-K solutions, each with **Plausibility** header (`objectiveValue`) and an icon | action | military subtotal table plus explained-vs-observed footer. **Ship-first near-solution** headers include a family chip (`Mine leftover` / `Ammo top-up`); a mixed-list subtitle appears when both families are present (shipped #415).
...
| `recentMinefieldObservationTurns` | Default **3**. Host-turn window for **recent minefield observation**. Regime entry (i) is any owner field `units > 0` in that window. `largeMinefieldObservationMinUnits` is dropped (#411). |
...
4. If INFEASIBLE and `alpha > 0`, **band retry** on military score only: `explained_2x >= observed_2x - alpha`; warship and freighter stay exact. **In-regime ship-first prefix does not take this branch** (item 15 / design §3.10).
5. **Exact solutions** from any step merge into **inference merged top-K** immediately (user-facing). **Do not** carry exact solutions forward as seeds -- no remainder to refine. Merge rules:
   ...
   - **K-best retention:** default top-K is 20. When *K* distinct signatures are held, the ladder **continues** climbing tiers (do not stop solely because the buffer is full). A new signature is admitted only if its **inference solution rank weight** exceeds the current worst held row, which is then evicted. **In-regime ship-first lists** use **ship-first stratified hold** instead (item 16 / design §3.11).
...
10. **Inference expensive-tier abort (shipped #368; entry-gate overlay shipped #411; leftover-0 skip / prefix shipped #413):** after cheap steps through `full_components`, if there is no hard-equality exact and the **hopeless classifier** fires, do not enter `admit_starbase_defense_posts` / `torp_escape_tier` / `full_catalog_exact`. Shipped classifier and N-window: design §3.5 and [Mine-score residual likelihood and expensive-tier abort](https://github.com/SteveDraper/Planets-Console/issues/357). **#411:** fold the 1000-unit size gate; abort follows **mine-contaminated regime** entry (any owner field `units > 0` in the window, decrease-shaped leftover beyond the moderate floor with mine-plausible counts, or sticky) and still refuses the expensive tail. Moderate 1-11 with no (i)/(iii) still aborts without sticky. YAML `largeMinefieldObservationMinUnits` dropped. **#413:** leftover-0 skip and ship-first prefix. Distinct from items 8-9 and from [#244](https://github.com/SteveDraper/Planets-Console/issues/244) per-step entry gates.
11. **Unknown military ship placeholder (shipped, #369):** after a no-list outcome (empty-list residual / moderate / `no_exact_solution`), emit post-unsat placeholders (not catalog combos, not `solutions[]`). Not emitted when the row holds **ship-first near-solution**s. Contract: design §3.6 and [Unknown military ship placeholder contract](https://github.com/SteveDraper/Planets-Console/issues/358).
12. **Ship loss, gift, and trade (shipped, #370; mixed gift+build #387):** admit decrease / pairing / **acquired ship** families and lattice-gated **idle-dock PP equality** from the first ship-bearing cheap step (superset thereafter). Pairing also uses idle-dock PP vs net (and dock-cap `net > starbases`) so mixed build+transfer is not cancelled in the raw count columns. Several `excess_out` peers are alternative donors for the same arrival budget -- a solution pairs with one, not a joint matching that consumes every peer. Emitted `militaryScoreArithmetic` tightens interval-action envelopes to leftover after the other elements of that solution; the catalog envelope stays the search domain. Contract: design §3.7 and [Ship loss, gift, and trade as exact families](https://github.com/SteveDraper/Planets-Console/issues/359). Prevents ship-only empty-exact on loss+replace. Post-limit PP is [#364](https://github.com/SteveDraper/Planets-Console/issues/364). Unpaired incoming stays `no_exact_solution`. Leftover odd PP after idle-dock (kill/recycle) is a follow-on.
13. **Product status persist (shipped, #366) and stream/SPA chrome (shipped, #367; ship-first residual persist shipped #413; chrome shipped #415; `uncharacterized_roster` persist shipped #489; roster chrome shipped #491):** first-class residual / skip / roster statuses on the persist write gate; skip rows fallback-complete; sticky prior from T-1 `mine_score_residual` including when that row holds **ship-first near-solution**s. Empty-list `complete` carries leftover + `placeholders[]`. **Mine-score residual** with a non-empty ship-first list persists ranked `solutions[]` + rank-1 leftover and `placeholders []`; those lists ride on `complete` only (no in-flight `solution` events). Roster terminals persist tagged leftover + lattice signatures and do not start sticky. Exact-set persist names `departurePins`. BFF `displayStatus` stays `mine_score_residual` for that status; `uncharacterized_roster` maps to itself. SPA: exact green N; residual with N>0 blue N (leftover in tooltip/aria); empty-list mine / moderate keep markers + leftover. Roster: distinct non-exact marker, bound leftover as `≥N`, lattice signatures in the existing modal without plausibility. Non-empty `placeholders[]` shipped by #369. Contract: design §3.8, [Inference product-status persist and stream contract](https://github.com/SteveDraper/Planets-Console/issues/360), and [Residual rows with ranked near-solutions: persist, stream, and SPA](https://github.com/SteveDraper/Planets-Console/issues/402).
14. **Worthwhile remainder bound (loader shipped #412; CP-SAT shipped #413):** in-regime ship-first overshoot is a hard cap, not score band. Exact-turn p90 of positive-stock units and field count, equal-split decay to leftover 2x, **remainder-bound turn mixture** (`n_total/(n_total+20)`, same-race isotonic leftover vs T, hold last knot), **observed-stock floor** from current-turn visible owner fields (exact per-field decay). Solve-time `cap_2x = floor(bound)` from `assets/analytics/scores/mine_stock_{category}.yaml`; diagnostics record the bound; the overshoot overlay uses that cap. Contract: design §3.9.
15. **Ship-first overshoot constraint (shipped #413):** runtime overlay on each prefix step through `admit_ship_torpedoes`, replacing band retry. Military window `observed + slack < explained <= observed + cap_2x`; warship/freighter stay equalities. Not `alpha`, not widened partition slack. Current-turn owner fields with `units > 0` skip leftover-0 exact. Otherwise exact-preempt, then overshoot-if-unsat. No in-regime under-explain retry; overshoots are not seeds. Same maximize-weight + near-best as exact; leftover is post-sort only. In-regime K-eviction: item 16. Contract: design §3.10.
16. **Ship-first family tag and stratified hold (shipped #414; chrome shipped #415):** belief torps stay in the prefix catalog (YAML 40 / diversity 2). No extra hard torp-military fraction of the **worthwhile remainder bound**. Core structural tag on each in-regime solution: non-torp military (exclude `ship_torps_loaded_*`) `> observed + slack` → `mine_overshoot`, else `ammo_top_up` when torps lift into the window. Wire `shipFirstFamily`. Stratified hold when both families have hits: floor 3 each (or all hits if fewer); rest of K by weight then leftover. One family may fill all K. SPA chips / mixed tooltip / modal subtitle shipped #415. Contract: design §3.11.
17. **Pinned vs uncharacterized departure (design #483; pin determination shipped #486; SAT admission shipped #487; emit shipped #488; persist/stream/export shipped #489; fleet retire shipped #490; chrome shipped #491):** `count_lattice.py` owns **count-lattice signal** and **departure pin**. `military_sat_admission.py` refuses `tier_solve` military catalog when a leave-signal on a military-moving class is unpinned. Case-2 product is `uncharacterized_roster` with **placeholder departure**s, tagged **unknown-loss leftover**, and **lattice signature**s (`uncharacterized_roster.py`; not `solutions[]`). Durable persist writes that status through the ADR 0002 gate; exact-set persist names `departurePins` (spec-only omits ids). Unique-fill persist is not a pin and does not open extra-warship capacity. Fleet@T retires named exact-set records to `lost` / `traded` and does not ingest placeholder departures as inferred acquisitions. Chrome: distinct cell marker, bound leftover as a lower bound, lattice signatures in the modal without plausibility. Combat/explosion extras: #485. Contract: design §3.12.
...
| **Inference moderate residual** / empty-list **mine-score residual** / `no_exact_solution` (stream/SPA chrome shipped, #367; abort classifier shipped, #368; placeholders shipped #369) | Distinct statuses; leftover + `placeholders[]` on persist / `complete`; not an inexact action list. Design §3.6 / §3.8. Cheap-unsat rows take **inference expensive-tier abort** when the hopeless classifier fires. |
| **Mine-score residual** with **ship-first near-solution**s (#402 persist shipped #413 / overshoot encoding #401 / family tag shipped #414 / chrome shipped #415) | Status stays `mine_score_residual`; ranked `solutions[]` + `shipFirstFamily` + rank-1 leftover; `placeholders []`. Blue count badge (N first; mix in tooltip). List on `complete` only. Moderate residual does not get this list. Overshoot sits in the **ship-first overshoot constraint** window (design §3.10) inside the **worthwhile remainder bound** (design §3.9 / #399). Families and stratified hold: design §3.11. Design §3.8. |
| **Uncharacterized roster** (design #483; chrome shipped #491) | Status `uncharacterized_roster`; `solutions[]` empty; **placeholder departure** + **lattice signature**s + tagged **unknown-loss leftover**. Distinct non-exact cell; bound leftover as `≥N`. Not `exact`. Design §3.12. |
...
- **Count-delta bound:** warship and freighter build actions are bounded by observed count deltas plus prior-fleet departure capacity, minus reserved incoming **acquired ship** counts (§3.7). Replacement combo slots (**extra warship/freighter capacity**) come from **known spec** decrease candidates only. Unique-fill persist and envelope-only fans are not a **departure pin** and do not open the eligibility catalog. A **count-lattice signal** with no pin on a military-moving class refuses military SAT entirely (#487); an unpinned true-freighter drop still SAT. Do not replay 683364 combo counts as assertions.
...
- mine laying and scooping (out of the #352 quality bar and out of [#394](https://github.com/SteveDraper/Planets-Console/issues/394); **mine-score residual** may carry **ship-first near-solution**s, which is not laying/scooping as catalog families),
