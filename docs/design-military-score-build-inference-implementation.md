...
17. **Pinned vs uncharacterized departure (design #483; pin determination shipped #486; SAT admission shipped #487; emit shipped #488; persist/stream/export shipped #489; fleet retire shipped #490):** `count_lattice.py` owns **count-lattice signal** and **departure pin**. `military_sat_admission.py` refuses `tier_solve` military catalog when a leave-signal on a military-moving class is unpinned. Case-2 product is `uncharacterized_roster` with **placeholder departure**s, tagged **unknown-loss leftover**, and **lattice signature**s (`uncharacterized_roster.py`; not `solutions[]`). Durable persist writes that status through the ADR 0002 gate; exact-set persist names `departurePins` (spec-only omits ids). Unique-fill persist is not a pin and does not open extra-warship capacity. Fleet@T retires named exact-set records to `lost` / `traded` and does not ingest placeholder departures as inferred acquisitions. Chrome remains follow-on #491. Combat/explosion extras: #485. Contract: design §3.12.
...
| **Uncharacterized roster** (design #483; chrome follow-on) | Status `uncharacterized_roster`; `solutions[]` empty; **placeholder departure** + **lattice signature**s + tagged **unknown-loss leftover**. Not `exact`. Design §3.12. |
