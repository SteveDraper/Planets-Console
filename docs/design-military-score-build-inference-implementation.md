...
17. **Pinned vs uncharacterized departure (design #483; pin determination shipped #486; SAT admission shipped #487; emit shipped #488):** `count_lattice.py` owns **count-lattice signal** and **departure pin**. `military_sat_admission.py` refuses `tier_solve` military catalog when a leave-signal on a military-moving class is unpinned. Case-2 in-memory product is `uncharacterized_roster` with **placeholder departure**s, tagged **unknown-loss leftover**, and **lattice signature**s (`uncharacterized_roster.py`; not `solutions[]`, not persisted). Unique-fill persist is not a pin and does not open extra-warship capacity. Persist, fleet retire, and chrome remain follow-ons #489–#491. Combat/explosion extras: #485. Contract: design §3.12.
