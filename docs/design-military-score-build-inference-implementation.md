...
| **Mine-score residual** with **ship-first near-solution**s (#402 persist shipped #413 / overshoot encoding #401 / family tag shipped #414) | Status stays `mine_score_residual`; ranked `solutions[]` + `shipFirstFamily` + rank-1 leftover; `placeholders []`. Blue count badge (N first; mix in tooltip) is #415. List on `complete` only. Moderate residual does not get this list. Overshoot sits in the **ship-first overshoot constraint** window (design §3.10) inside the **worthwhile remainder bound** (design §3.9 / #399). Families and stratified hold: design §3.11. Design §3.8. |
