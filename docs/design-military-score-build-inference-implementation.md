...
17. **Pinned vs uncharacterized departure (design #483; pin determination shipped #486):** `count_lattice.py` owns **count-lattice signal** and **departure pin** (alibi cull, exact-set vs spec-only **known spec**). Unique-fill persist is not a pin. SAT admission, uncharacterized-roster emit, persist, fleet retire, and chrome remain follow-ons #487–#491. Combat/explosion extras: #485. Contract: design §3.12.
