...
13. **Product status persist (shipped, #366) and stream/SPA chrome (shipped, #367):** first-class residual / skip statuses on the persist write gate; skip rows fallback-complete; sticky prior from T-1 `mine_score_residual`. Table-stream `complete` carries leftover + empty `placeholders[]`; BFF `displayStatus` adds `skipped` / residual buckets; SPA skip mute, residual markers, and Stealth Mode grey **Include build inference**. Non-empty `placeholders[]` is #369. Contract: design §3.8 and [Inference product-status persist and stream contract](https://github.com/SteveDraper/Planets-Console/issues/360).
...
| **Inference moderate residual** / **mine-score residual** / `no_exact_solution` (stream/SPA chrome shipped, #367; abort classifier shipped, #368; non-empty placeholders #369) | Distinct statuses (`moderate_residual` / `mine_score_residual` / `no_exact_solution`); leftover + empty `placeholders[]` on persist / `complete`; not an inexact action list. Design §3.6 / §3.8. Cheap-unsat rows take **inference expensive-tier abort** when the hopeless classifier fires. |
