...
13. **Product status persist (shipped, #366):** first-class residual / skip statuses on the persist write gate; skip rows fallback-complete; sticky prior from T-1 `mine_score_residual`. Stream `complete` payload, `placeholders` on `complete`, and SPA cell chrome remain the next phase. Contract: design §3.8 and [Inference product-status persist and stream contract](https://github.com/SteveDraper/Planets-Console/issues/360).
