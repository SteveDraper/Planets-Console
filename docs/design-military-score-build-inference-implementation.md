...
| `recentMinefieldObservationTurns` | Default **3**. Host-turn window for **recent minefield observation**. |
| `largeMinefieldObservationMinUnits` | Default **1000**. Shipped #368 abort size gate. Folded by [#394](https://github.com/SteveDraper/Planets-Console/issues/394) / [#403](https://github.com/SteveDraper/Planets-Console/issues/403): not a current glossary term; drop in the later implementation epic. Regime entry (i) is any `units > 0` in the **recent minefield observation** window. |
...
10. **Inference expensive-tier abort (shipped #368; overlay #394 / #403):** after cheap steps through `full_components`, if there is no hard-equality exact and the **hopeless classifier** fires, do not enter `admit_starbase_defense_posts` / `torp_escape_tier` / `full_catalog_exact`. Shipped classifier, window, and 1000-unit size gate: design §3.5 and [Mine-score residual likelihood and expensive-tier abort](https://github.com/SteveDraper/Planets-Console/issues/357). **#394 overlay (later implementation):** fold that size gate; in-regime abort follows **mine-contaminated regime** entry and still refuses the expensive tail. Distinct from items 8-9 and from [#244](https://github.com/SteveDraper/Planets-Console/issues/244) per-step entry gates.
