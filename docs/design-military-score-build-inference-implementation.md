...
13. **Product status persist (shipped, #366) and stream/SPA chrome (shipped, #367; ship-first residual persist shipped #413; chrome shipped #415):** first-class residual / skip statuses on the persist write gate (statuses unchanged); skip rows fallback-complete; sticky prior from T-1 `mine_score_residual` including when that row holds **ship-first near-solution**s. Empty-list `complete` carries leftover + `placeholders[]`. **Mine-score residual** with a non-empty ship-first list persists ranked `solutions[]` + rank-1 leftover and `placeholders []`; those lists ride on `complete` only (no in-flight `solution` events). BFF `displayStatus` stays `mine_score_residual` for that status. SPA: exact green N; residual with N>0 blue N (leftover in tooltip/aria); empty-list mine / moderate keep markers + leftover. Non-empty `placeholders[]` shipped by #369. Contract: design §3.8, [Inference product-status persist and stream contract](https://github.com/SteveDraper/Planets-Console/issues/360), and [Residual rows with ranked near-solutions: persist, stream, and SPA](https://github.com/SteveDraper/Planets-Console/issues/402).
...
17. **Pinned vs uncharacterized departure (design #483; not shipped):** **count-lattice signal** and **departure pin** (alibi cull, exact-set vs spec-only **known spec**) gate military SAT. Unpinned military-moving class: no `tier_solve` catalog; **uncharacterized roster**; **placeholder departure**; **unknown-loss leftover**; **lattice signature**s not in `solutions[]`. Exact-set pin retires fleet rows at fleet@T. Unique-fill persist is not a pin. Follow-ons: #486–#491. Combat/explosion extras: #485. Contract: design §3.12.
...
- **Count-delta bound:** warship and freighter build actions are bounded by observed count deltas plus prior-fleet departure capacity, minus reserved incoming **acquired ship** counts (§3.7). **Current (#482 lock):** departure capacity unique-fills only when a prior record has a single option hull (or known fields) that can complete engine/slots, or when option sets carry a military envelope. A multi-hull fan with no envelopes is not a decrease candidate, so `warship_delta == 0` collapses the next-turn warship product to empty. A singleton persist of the same ship *opens* a replacement slot and admits the full eligibility catalog. Unique-fill does not pin the replacement product to that hull. **Target (#483 §3.12):** unique-fill is not a **departure pin**; an unpinned military-moving class refuses SAT and must not open that catalog. The SAT-admission follow-on replaces the #482 tests.
