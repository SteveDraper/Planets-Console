...
| **Inference moderate residual** / **mine-score residual** / `no_exact_solution` (stream/SPA chrome shipped, #367; non-empty placeholders #369) | Distinct statuses (`moderate_residual` / `mine_score_residual` / `no_exact_solution`); leftover + empty `placeholders[]` on persist / `complete`; not an inexact action list. Design §3.6 / §3.8. Production still rarely emits residual statuses until the abort classifier (#368). |
