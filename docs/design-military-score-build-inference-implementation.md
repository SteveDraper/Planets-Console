...
10. **Inference expensive-tier abort (shipped #368; entry-gate overlay shipped #411):** after cheap steps through `full_components`, if there is no hard-equality exact and the **hopeless classifier** fires, do not enter `admit_starbase_defense_posts` / `torp_escape_tier` / `full_catalog_exact`. Shipped classifier and N-window: design §3.5 and [Mine-score residual likelihood and expensive-tier abort](https://github.com/SteveDraper/Planets-Console/issues/357). **#411:** fold the 1000-unit size gate; abort follows **mine-contaminated regime** entry (any owner field `units > 0` in the window, decrease-shaped leftover beyond the moderate floor with mine-plausible counts, or sticky) and still refuses the expensive tail. Moderate 1-11 with no (i)/(iii) still aborts without sticky. YAML `largeMinefieldObservationMinUnits` dropped. Leftover-0 skip and ship-first prefix remain later ([#413](https://github.com/SteveDraper/Planets-Console/issues/413)). Distinct from items 8-9 and from [#244](https://github.com/SteveDraper/Planets-Console/issues/244) per-step entry gates.
...
| **Mine-score residual** with **ship-first near-solution**s (#402 / overshoot encoding #401 / family tag #400; implementation later on #394) | Status stays `mine_score_residual`; ranked `solutions[]` + rank-1 leftover + `shipFirstFamily`; `placeholders []`. Blue count badge (N first; mix in tooltip). List on `complete` only. Moderate residual does not get this list. Overshoot sits in the **ship-first overshoot constraint** window (design §3.10) inside the **worthwhile remainder bound** (design §3.9 / #399). Families and stratified hold: design §3.11. Design §3.8. |
