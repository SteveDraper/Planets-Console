...
| **Mine-score residual** with **ship-first near-solution**s (#402; implementation later on #394) | Status stays `mine_score_residual`; ranked `solutions[]` + rank-1 leftover; `placeholders []`. Blue count badge (N first). List on `complete` only. Moderate residual does not get this list. Overshoot must sit inside the **worthwhile remainder bound** (design §3.9 / #399). Design §3.8. |
