...
17. **Pinned vs uncharacterized departure (design #483; pin determination shipped #486; SAT admission shipped #487; emit shipped #488; persist/stream/export shipped #489):** `count_lattice.py` owns **count-lattice signal** and **departure pin**. `military_sat_admission.py` refuses `tier_solve` military catalog when a leave-signal on a military-moving class is unpinned. Case-2 product is `uncharacterized_roster` with **placeholder departure**s, tagged **unknown-loss leftover**, and **lattice signature**s (`uncharacterized_roster.py`; not `solutions[]`). Durable persist writes that status through the ADR 0002 gate; exact-set persist names `departurePins` (spec-only omits ids). Unique-fill persist is not a pin and does not open extra-warship capacity. Fleet retire and chrome remain follow-ons #490–#491. Combat/explosion extras: #485. Contract: design §3.12.
