...
**SPA chrome** (not modal layout). BFF `displayStatus` stays `mine_score_residual` when `status` is that value (does not flip to `success` because `solutionCount > 0`). Skip: muted cell, tooltip, no modal. Exact: green solid N. Complete **mine-score residual** with **N > 0**: the same **inference solution count indicator** in blue (probable build first; leftover secondary -- tooltip / accessible name, not a second numeral). Empty-list **mine-score residual** and **inference moderate residual**: distinct residual markers + leftover size. `no_exact_solution` keeps the red X. Click opens the existing **inference solution detail modal**: ranked list + explained-vs-observed mismatch when **N > 0**; summary when the list is empty. Do not design a new modal.
...
In the **mine-contaminated regime**, a **ship-first near-solution** is feasible only when overshoot leftover satisfies `0 < leftover_2x <= worthwhile remainder bound` (leftover 0 is `exact`). Rank by **inference solution rank weight**; leftover is a tie-break and this hard cap, not the primary objective. The cap is not **inference score band** and not a ranking-only penalty.
