...
| **Uncharacterized roster** ([§3.12](#312-pinned-vs-uncharacterized-departure); chrome shipped #491) | `uncharacterized_roster` |
...
**Export.** `$.meta.searchStatus` stays lifecycle-only (`complete` for these terminals). Product `status`, leftover (point or **unknown-loss leftover**), `placeholders`, and **lattice signature**s are siblings of `$.solutions`. `$.solutions` may be non-empty on `mine_score_residual`. Shipped by [Scores analytic exports](https://github.com/SteveDraper/Planets-Console/issues/97); tagged leftover and `uncharacterized_roster` persist/stream/export shipped [#489](https://github.com/SteveDraper/Planets-Console/issues/489). Export precedence is unchanged.

**SPA chrome** (not modal layout). BFF `displayStatus` stays `mine_score_residual` when `status` is that value (does not flip to `success` because `solutionCount > 0`). Skip: muted cell, tooltip, no modal. Exact: green solid N. Complete **mine-score residual** with **N > 0**: the same **inference solution count indicator** in blue (probable build first; leftover secondary -- tooltip / accessible name, not a second numeral). When the list mixes **mine-overshoot** and **ammo-top-up**, tooltip/aria names the mix and still quotes rank-1 leftover. Empty-list **mine-score residual** and **inference moderate residual**: distinct residual markers + leftover size. `no_exact_solution` keeps the red X. **Uncharacterized roster** ([#491](https://github.com/SteveDraper/Planets-Console/issues/491) shipped): BFF `displayStatus` `uncharacterized_roster` (not `success`, not skipped, not red-X `failure`); cell is a distinct non-exact marker plus leftover; bound leftover shows as a lower bound (`≥N`) whose tooltip/aria names unknown lost-ship contribution; click opens the existing **inference solution detail modal** with empty `solutions[]`, **placeholder departure**s, **lattice signature**s without `objectiveValue` / plausibility, and point vs bound leftover. Do not silently reuse `exact` or render a bound as a point leftover. Ranked-list click path is unchanged: one weight-ordered list + explained-vs-observed mismatch when **N > 0**; **ship-first family tag** chip on each header; mixed-list subtitle when both families are present; summary when the list is empty. Do not design a new modal, split N, family sections, or leftover-on-cell beside N. Family tag / **ship-first stratified hold**: [§3.11](#311-ship-first-family-tag-and-stratified-hold).

### 3.9 Worthwhile remainder bound

Locked in [Remainder bound percentile and turn partitioning](https://github.com/SteveDraper/Planets-Console/issues/399). Arithmetic: [Mine-stock to leftover military arithmetic](https://github.com/SteveDraper/Planets-Console/issues/397). Asset grain: [Mine-stock prior mining surface](https://github.com/SteveDraper/Planets-Console/issues/396) / [Collect mine-stock histograms from finished games](https://github.com/SteveDraper/Planets-Console/issues/398). Map: [#394](https://github.com/SteveDraper/Planets-Console/issues/394). **Loader and solve-time bound shipped** ([Implement worthwhile remainder bound](https://github.com/SteveDraper/Planets-Console/issues/412)). **CP-SAT overshoot window shipped** ([Implement ship-first overshoot constraint and residual persist](https://github.com/SteveDraper/Planets-Console/issues/413)).
...
Locked in [Belief-torp remainder cap against the worthwhile bound](https://github.com/SteveDraper/Planets-Console/issues/400). Window: [§3.10](#310-ship-first-overshoot-constraint). Persist/chrome: [§3.8](#38-product-status-persist-and-stream) / [#402](https://github.com/SteveDraper/Planets-Console/issues/402). Map: [#394](https://github.com/SteveDraper/Planets-Console/issues/394). **Shipped** ([Implement ship-first family tag and stratified hold](https://github.com/SteveDraper/Planets-Console/issues/414)). SPA chips / mixed tooltip / modal subtitle shipped ([Implement ship-first residual stream and SPA chrome](https://github.com/SteveDraper/Planets-Console/issues/415)).
...
Locked in [Pinned vs uncharacterized departure: design (docs only)](https://github.com/SteveDraper/Planets-Console/issues/483) (**design only**). Implementation follow-ons: pin [#486](https://github.com/SteveDraper/Planets-Console/issues/486) (shipped), SAT admission [#487](https://github.com/SteveDraper/Planets-Console/issues/487) (shipped), emit [#488](https://github.com/SteveDraper/Planets-Console/issues/488) (shipped), persist [#489](https://github.com/SteveDraper/Planets-Console/issues/489) (shipped), fleet [#490](https://github.com/SteveDraper/Planets-Console/issues/490) (shipped), chrome [#491](https://github.com/SteveDraper/Planets-Console/issues/491) (shipped). Combat / explosion report extras: [#485](https://github.com/SteveDraper/Planets-Console/issues/485). Unique-fill persist is not a **departure pin** and does not open replacement catalog width.
...
**SPA chrome** shipped [#491](https://github.com/SteveDraper/Planets-Console/issues/491): must not reuse `exact`, must not render a bound as a point leftover, must surface lattice signatures without plausibility ranking. Design-doc names the wire.

**Tests (implementation follow-ons, not this design PR).** Table-driven leave-signal x pin/fail (alibi, exact-set, spec-only, unique-fill is not a pin, unpinned sibling fails). Lattice emit (net -1 warship, idle-dock 0-build +1 PP with both class signatures, gift count + counterparty). Unique-fill persist combo does not pin. Case 1 still SAT when fields are RST-known. Do not replay 683364 combo counts as assertions. SAT admission tests (#487) replace the #482 unique-fill catalog-width lock.
...
| Hopeless classifier / expensive-tier abort (#357 / shipped #368; entry-gate overlay shipped #411; leftover-0 skip / prefix shipped #413) | **Shipped:** cheap always through `full_components` out of regime. Abort `admit_starbase_defense_posts`+ on cheap-unsat if decrease-shaped >11, sticky prior, or any owner field `units > 0` in the N-window (`recentMinefieldObservationTurns`, default 3). Moderate 1-11 with no (i)/(iii) aborts expensive without starting sticky. Not #244. **#411:** folded the 1000-unit gate; YAML `largeMinefieldObservationMinUnits` dropped. **#413:** in-regime prefix through `admit_ship_torpedoes` (no planet/SB posts); current-turn owner fields skip leftover-0. |
...
| Product status persist and stream (#360; ship-first residual list #402; persist shipped #413; `uncharacterized_roster` persist shipped #489; chrome shipped #491) | First-class `status` (`exact`, `moderate_residual`, `mine_score_residual`, `no_exact_solution`, `uncharacterized_roster`, per-reason skip). Write-gate includes `uncharacterized_roster`. Empty-list residuals functional-persist with `placeholders` + leftover; **mine-score residual** with **ship-first near-solution**s persists ranked `solutions[]` + rank-1 leftover and `placeholders []` (#413). Roster leftover is tagged **unknown-loss leftover** plus **lattice signature**s; exact-set persist names `departurePins`. Skips fallback-complete. Sticky derived from T-1 `mine_score_residual` (including non-empty list); leftover within slack is `exact` and clears sticky except when current-turn owner fields skip leftover-0 (§3.10). `uncharacterized_roster` does not start sticky. Stream: incremental `solution` events leftover-0 / exact only; overshooting lists on `complete` only (SPA chrome shipped #415); roster terminals are one `complete` with no `solution` events. BFF `displayStatus` stays `mine_score_residual` when that is `status`; `uncharacterized_roster` maps to itself (not `success`). Exact: green N. Residual with N>0: same badge in blue (leftover in tooltip/aria). Empty-list mine / moderate: marker + leftover. Roster: distinct non-exact marker; bound leftover as `≥N` with unknown lost-ship aria. No new modal. Export: product `status` / leftover / `placeholders` / `latticeSignatures` siblings of `$.solutions` (solutions may be non-empty on mine residual); `searchStatus` stays lifecycle-only. |
...
| Pinned vs uncharacterized departure (#483 design; pin #486 / SAT #487 / emit #488 / persist #489 / fleet #490 / chrome #491 shipped) | **Count-lattice signal** from pairing / idle-dock, not unique-fill. **Departure pin** after **fleet alibi** cull: exact-set or spec-only **known spec**; last-turn inference singleton is not a pin. Refuse military SAT only when an unpinned class can move military. Case 2: **uncharacterized roster**, **placeholder departure**, **unknown-loss leftover**, **lattice signature**s (not `solutions[]`); persist/stream/export shipped #489. Exact-set pin names persist `departurePins`; fleet@T retires those records (`lost` / `traded`); spec-only omits ids. Placeholder departures are not ingested as inferred acquisitions. Chrome shipped #491. Combat/explosion extras: #485. §3.12. |
