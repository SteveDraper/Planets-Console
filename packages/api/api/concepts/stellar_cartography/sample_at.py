...
def _star_cluster_entries(turn: TurnInfo, x: int, y: int) -> list[dict]:
    ...
    for name, bodies in by_name.items():
        lethal = False
        lethal_temp: int | None = None
        max_radiation = 0
        for body in bodies:
            dist = _distance_ly(x, y, body.x, body.y)
            if dist <= body.radius:
                lethal = True
                lethal_temp = body.temp
                break
            halo = math.sqrt(body.mass)
            if dist < halo:
                rad = math.ceil((body.temp / 100) * (1.0 - dist / halo))
                max_radiation = max(max_radiation, int(rad))
        if lethal:
            temp_label = f"temp {lethal_temp}" if lethal_temp is not None else "temp —"
            entries.append(
                {
                    "layer": LAYER_STAR_CLUSTERS,
                    "lines": [f"{name} — lethal — {temp_label}"],
                }
            )
        elif max_radiation > 0:
            entries.append(
                {
                    "layer": LAYER_STAR_CLUSTERS,
                    "lines": [f"{name} — radiation {max_radiation}"],
                }
            )
    return entries
...
def sample_at(turn: TurnInfo, x: int, y: int) -> dict:
    ...
    by_layer: dict[str, list[dict]] = {
        ...
        LAYER_STAR_CLUSTERS: _star_cluster_entries(turn, x, y),
