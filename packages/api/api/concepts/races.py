...
DEFAULT_PREFERRED_HOMEWORLD_TEMP_W = 50
CRYSTAL_DESERT_PREFERRED_HOMEWORLD_TEMP_W = 100
...
def is_crystal(race_id: int) -> bool:
    return race_id == CRYSTAL_RACE_ID


def preferred_homeworld_temp_w(race_id: int) -> int:
    """Race climate catalog preferred planet temperature for homeworld baseline profile.

    Crystal defaults to 100 deg W (desert peak). Per-game desert-advantage settings
    probe is deferred -- Crystal always uses the desert preferred temp until then.
    """
    if race_id == CRYSTAL_RACE_ID:
        return CRYSTAL_DESERT_PREFERRED_HOMEWORLD_TEMP_W
    return DEFAULT_PREFERRED_HOMEWORLD_TEMP_W
